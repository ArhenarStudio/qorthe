  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-serif text-3xl text-[#2d2419]">Centro de Envíos</h1>
          <p className="text-[14px] text-[#8D6E63] mt-1">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div className="bg-white border border-[#EFEBE9] rounded-lg p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[24px] font-bold text-[#2d2419]">{pendingOrders.length}</p>
              <p className="text-[12px] text-[#A1887F]">Pendientes de envío</p>
            </div>
          </div>
        </div>
        <div className="bg-white border border-[#EFEBE9] rounded-lg p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[24px] font-bold text-[#2d2419]">{shippedOrders.length}</p>
              <p className="text-[12px] text-[#A1887F]">Enviados</p>
            </div>
          </div>
        </div>
        <div className="bg-white border border-[#EFEBE9] rounded-lg p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[24px] font-bold text-[#2d2419]">
                {pendingOrders.length + shippedOrders.length}
              </p>
              <p className="text-[12px] text-[#A1887F]">Total órdenes</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {loading && (
        <div className="text-center py-16">
          ...
          <p className="text-[14px] text-[#8D6E63]">Cargando órdenes...</p>
        </div>
      )}
        ...
        <div className="bg-white border border-[#EFEBE9] rounded-lg p-12 text-center">
          {activeTab === "pending" ? (
            <>
              ...
              <h2 className="font-serif text-xl text-[#2d2419] mb-2">
                Todo al día
              </h2>
            ...
            <>
              <Package size={40} className="mx-auto mb-4 text-[#BCAAA4]" />
              <h2 className="font-serif text-xl text-[#2d2419] mb-2">
                Sin envíos aún
              </h2>
              <p className="text-[14px] text-[#8D6E63]">
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className="w-full flex items-center gap-4 p-4 hover:bg-[#FAFAF8] transition-colors text-left"
                >
                  {/* Status dot */}
                  <div
                  ...
                  <div className="w-28 flex-shrink-0">
                    <p className="font-bold text-[14px] text-[#2d2419]">
                      #{order.display_id}
                    </p>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] text-[#2d2419] truncate">
                      {addr.first_name} {addr.last_name}
                    </p>
                    <p className="text-[11px] text-[#A1887F] truncate">
                      {order.email}
                    </p>
                  </div>
                  ...
                  <div className="hidden md:block flex-1 min-w-0">
                    <p className="text-[13px] text-[#5D4037] truncate">
                  ...
                  <div className="w-8 flex-shrink-0 text-center">
                    {order.fulfillment ? (
                      ...
                      <ChevronUp size={16} className="text-[#A1887F] mx-auto" />
                    ) : (
                      <ChevronDown size={16} className="text-[#A1887F] mx-auto" />
                    )}
                  </div>
                </button>
                ...
                {/* Expanded detail */}
                {isExpanded && (
                  <div className="border-t border-[#EFEBE9] p-5 bg-[#FAFAF8]">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      ...
                      <div>
                          ...
                          {order.items.map((item) => (
                            ...
                            >
                              {item.thumbnail ? (
                                ...
                                <div className="w-10 h-10 rounded bg-[#D7CCC8]" />
                              )}
                              <div className="flex-1 min-w-0">
                                <p className="text-[13px] text-[#2d2419] truncate font-medium">
                                  {item.title}
                                </p>
                                {item.variant_title && (
                                  <p className="text-[11px] text-[#A1887F]">
                              ...
                              <div className="text-right">
                                <p className="text-[13px] text-[#2d2419]">
                                  ×{item.quantity}
                                </p>
                                <p className="text-[11px] text-[#A1887F]">
                        ...
                        {/* Totals */}
                        <div className="mt-3 pt-3 border-t border-[#EFEBE9] space-y-1">
                          <div className="flex justify-between text-[12px]">
                            <span className="text-[#A1887F]">Productos</span>
                            <span className="text-[#2d2419]">{formatPrice(order.item_total)}</span>
                          </div>
                          <div className="flex justify-between text-[12px]">
                            <span className="text-[#A1887F]">Envío</span>
                            <span className="text-[#2d2419]">{formatPrice(order.shipping_total)}</span>
                          </div>
                          <div className="flex justify-between text-[13px] font-bold pt-1 border-t border-[#EFEBE9]">
                            <span className="text-[#2d2419]">Total</span>
                            <span className="text-[#2d2419]">{formatPrice(order.total)}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="bg-white rounded p-4 border border-[#EFEBE9] space-y-1">
                          ...
                          <p className="text-[13px] text-[#5D4037]">{addr.address_1}</p>
                          {addr.address_2 && (
                            <p className="text-[13px] text-[#5D4037]">{addr.address_2}</p>
                          )}
                          <p className="text-[13px] text-[#5D4037]">
                            {addr.city}, {addr.province} CP {addr.postal_code}
                          </p>
                          {addr.phone && (
                            <p className="text-[13px] text-[#8D6E63]">Tel: {addr.phone}</p>
                          )}
                          <p className="text-[12px] text-[#A1887F] pt-1">{order.email}</p>
                        </div>
                        ...
                        {order.shipping_method && (
                          <div className="mt-3 bg-white rounded p-3 border border-[#EFEBE9]">
                            ...
                            <p className="text-[13px] font-medium text-[#2d2419]">
                              {order.shipping_method.name}
                            </p>
                          </div>
                        )}

                        <p className="text-[11px] text-[#BCAAA4] mt-3">
                        ...
                        {!order.fulfillment && (
                          <div>
                            ...
                            <button
                              ...
                              className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-[#2d2419] text-white text-[13px] font-semibold rounded hover:bg-[#3e3226] transition-colors disabled:opacity-50"
                            >
                              {isGenerating ? (
                                <>
