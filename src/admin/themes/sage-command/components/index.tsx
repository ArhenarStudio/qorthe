...
export const SageButton: React.FC<{
  ...
  const variants: Record<string, React.CSSProperties> = {
    ...
    secondary: { backgroundColor: 'var(--surface2)', color: 'var(--text)', border: '1px solid var(--border)' },
    ghost:     { backgroundColor: 'transparent', color: 'var(--text-muted)', border: 'none' },
    danger:    { backgroundColor: 'var(--surface2)', color: 'var(--error)', border: '1px solid rgba(220,38,38,0.2)' },
  };
...
export const SageStatCard: React.FC<{ label: string; value: string | number; change?: string; changeType?: string; icon?: React.ReactNode }> = ({ label, value, change, changeType = 'neutral', icon }) => (
  <div className="p-4" style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-card)' }}>
    <div className="flex items-center justify-between mb-2">
      ...
      {icon && <span style={{ color: 'var(--accent)', opacity: 0.4 }}>{icon}</span>}
    </div>
