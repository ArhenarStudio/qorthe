...
export const SageCommandSidebar: React.FC<Props> = ({ currentPage, onNavigate, collapsed, onToggleCollapse, navigation }) => {
  ...
  return (
    ...
    >
      {/* Logo */}
      <div className="flex items-center gap-3 px-4 h-14" style={{ borderBottom: '1px solid var(--sidebar-border)' }}>
        <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0"
          style={{ background: 'linear-gradient(135deg, var(--accent), var(--info))', boxShadow: '0 2px 8px rgba(20,184,166,0.3)' }}>
          <Hexagon size={16} className="text-white" />
        </div>
        <AnimatePresence>
          {!collapsed && (
            <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="text-[13px] font-semibold" style={{ color: 'var(--sidebar-text)', fontFamily: 'var(--font-heading)' }}>
              RockSage
            </motion.span>
          )}
        </AnimatePresence>
          ...
          <div key={group.id}>
            {!collapsed && group.label && (
              <p className="px-2.5 pt-5 pb-1.5 text-[10px] uppercase tracking-[0.14em] font-medium"
                style={{ color: 'var(--text-muted)', fontFamily: 'var(--font-heading)' }}>{group.label}</p>
            )}
            {collapsed && group.id !== 'general' && <div className="mx-2 my-2" style={{ borderTop: '1px solid var(--sidebar-border)' }} />}
                ...
                return (
                  <div key={item.id} className="relative">
                    <button
                      ...
                      style={{
                        backgroundColor: isActive ? 'var(--sidebar-active)' : isHovered ? 'rgba(255,255,255,0.03)' : 'transparent',
                        color: isActive ? 'var(--accent)' : 'var(--text-muted)',
                      }}
                    >
                      <Icon size={16} strokeWidth={isActive ? 2 : 1.5} />
                      {!collapsed && (
                        <span className="truncate flex-1 text-left" style={{ color: isActive ? 'var(--sidebar-text)' : 'var(--text-secondary)', fontFamily: 'var(--font-body)' }}>{item.label}</span>
                      )}
                      {!collapsed && badge > 0 && (
          ...
          <AnimatePresence>
            {!collapsed && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 min-w-0">
                ...
                <p className="text-[10px]" style={{ color: 'var(--text-muted)' }}>admin@rocksage.com</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
