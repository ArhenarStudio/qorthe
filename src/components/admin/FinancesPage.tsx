...
const SectionTitle: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <h4 className="text-sm font-medium text-wood-900 uppercase tracking-wider border-b border-wood-100 pb-2 mb-4">{children}</h4>
);

// Donut component
const DonutChart: React.FC<{ data: { name: string; value: number; pct: number }[]; title: string }> = ({ data, title }) => (
  <Card className="p-5">
    <h5 className="text-xs font-medium text-wood-700 mb-3">{title}</h5>
    <div className="h-40">
      ...
      {data.map((d, i) => (
        <div key={d.name} className="flex items-center justify-between text-[11px]">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-wood-600 truncate">{d.name}</span>
          </div>
          <span className="font-medium text-wood-900">{d.pct}%</span>
        </div>
      ))}
    </div>
  </Card>
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <tbody>
              {plRows.map((r, i) => (
                <tr key={i} className={`${r.bold ? 'border-t border-wood-200' : ''} ${r.highlight ? 'bg-green-50' : ''}`}>
                  <td className={`py-1.5 pr-4 text-xs ${r.indent ? 'pl-6' : 'pl-0'} ${r.bold ? 'font-semibold text-wood-900' : 'text-wood-600'}`}>
                    {r.label}
                  </td>
                  <td className={`py-1.5 text-xs text-right font-mono tabular-nums ${r.bold ? 'font-semibold text-wood-900' : r.value < 0 ? 'text-red-600' : 'text-wood-700'}`}>
                    {r.value < 0 ? `-${fmt(Math.abs(r.value))}` : fmt(r.value)}
                  </td>
                  <td className="py-1.5 text-[10px] text-right text-wood-400 pl-3 w-16">{r.pct}%</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <p className="text-[10px] text-wood-400 mt-3 italic">* Costos operativos: hosting, herramientas, Neon DB, Vercel, DigitalOcean, dominio, insumos taller, mano de obra</p>
      </Card>
            ...
            <Card key={i} className="p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-lg font-semibold text-wood-900 font-sans">{m.value}</p>
                  <p className="text-[10px] text-wood-400 uppercase tracking-wider">{m.label}</p>
                  <p className="text-[10px] text-wood-500 mt-0.5">{m.sub}</p>
                </div>
              </div>
            </Card>
          ))}
      ...
      <Card className="p-5">
        ...
        <p className="text-xs text-wood-500 mb-4">Basado en tendencia de los últimos 6 meses + cotizaciones en pipeline</p>
        <div className="h-56">
          <ResponsiveContainer width="100%" height="100%">
    ...
    <div className="space-y-6">
            ...
            <tbody className="divide-y divide-wood-50">
              {d.topProductsByRevenue.map((p: any) => (
                <tr key={p.rank} className="hover:bg-sand-50/50 transition-colors">
                  ...
                  <td className="px-5 py-2.5 text-xs text-wood-600 text-right">{p.units}</td>
                  <td className="px-5 py-2.5 text-xs font-medium text-wood-900 text-right font-mono">{fmt(p.revenue)}</td>
                  <td className="px-5 py-2.5 text-xs text-wood-500 text-right">{p.pct}%</td>
                  ...
                  <td className="px-5 py-2.5 text-xs text-right">
                    ...
                    {p.trendDir === 'flat' && <span className="text-wood-400">+{p.trend}%</span>}
          ...
          {d.revenueBySubCategory.map((cat: any) => (
            <div key={cat.name}>
              <div className="flex items-center justify-between text-xs mb-1">
                ...
                <span className="text-wood-900 font-mono">{fmt(cat.total)} ({cat.pct}%)</span>
              </div>
              <div className="w-full bg-wood-50 rounded-full h-2.5">
                <div className="bg-accent-gold h-2.5 rounded-full transition-all" style={{ width: `${cat.pct}%` }} />
              </div>
              {'sub' in cat && cat.sub && (
                <div className="ml-4 mt-1.5 space-y-1">
                  {cat.sub.map((s: any) => (
                    <div key={s.name} className="flex items-center justify-between text-[10px]">
                      <span className="text-wood-500">{s.name}</span>
                      <span className="text-wood-600 font-mono">{fmt(s.value)} ({s.pct}%)</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            {d.revenueByChannel.map((ch: any) => (
              <div key={ch.name} className="flex items-center justify-between text-xs">
                <div>
                  ...
                  {'note' in ch && ch.note && <span className="text-[10px] text-wood-400 ml-1">({ch.note})</span>}
                </div>
                <span className="font-mono font-medium text-wood-900">{fmt(ch.value)} <span className="text-wood-400">({ch.pct}%)</span></span>
              </div>
            ))}
          </div>
        </Card>
            ...
            {d.revenueByRegion.map((r: any) => (
              <div key={r.name} className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-wood-700">{r.name}</span>
                </div>
                <div className="text-right">
                  <span className="font-mono font-medium text-wood-900">{fmt(r.value)} ({r.pct}%)</span>
                  <span className="text-[10px] text-wood-400 ml-2">Envío prom {fmt(r.avgShipping)}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <Card className="p-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div>
            <p className="text-[10px] text-wood-400 uppercase tracking-wider">Pedidos con grabado</p>
            <p className="text-lg font-semibold text-wood-900 font-sans">{d.engravingStats.ordersWithEngraving} de {d.engravingStats.totalOrders} ({d.engravingStats.pct}%)</p>
          </div>
          <div>
            <p className="text-[10px] text-wood-400 uppercase tracking-wider">Incremento promedio</p>
            <p className="text-lg font-semibold text-wood-900 font-sans">+{fmt(d.engravingStats.avgIncrease)} MXN</p>
          </div>
          <div>
            <p className="text-[10px] text-wood-400 uppercase tracking-wider">Ingresos adicionales</p>
            <p className="text-lg font-semibold text-wood-900 font-sans">{fmt(d.engravingStats.additionalRevenue)}</p>
          </div>
          <div>
            <p className="text-[10px] text-wood-400 uppercase tracking-wider">Margen del grabado</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="p-5">
          <h5 className="text-xs font-medium text-wood-700 mb-3">Distribución de costos totales</h5>
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
            ...
            {d.totalCostDonut.map((c: any, i: number) => (
              <div key={c.name} className="flex items-center justify-between text-[11px]">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-wood-600">{c.name}</span>
                </div>
                <span className="font-medium text-wood-900">{fmt(c.value)} ({c.pct}%)</span>
              </div>
            ))}
          </div>
        </Card>
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* COGS table */}
          <Card className="overflow-hidden">
            <div className="px-5 py-3 border-b border-wood-100"><SectionTitle>COGS — {fmt(d.cogs)}</SectionTitle></div>
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="text-[10px] text-wood-400 uppercase tracking-wider border-b border-wood-50">
                ...
                <tbody className="divide-y divide-wood-50">
                  {d.cogsByProduct.map((p: any) => (
                    <tr key={p.product} className="hover:bg-sand-50/50 transition-colors">
                      ...
                      <td className="px-4 py-2 text-xs text-wood-600 text-right font-mono">{p.costUnit > 0 ? fmt(p.costUnit) : 'var.'}</td>
                      <td className="px-4 py-2 text-xs font-medium text-wood-900 text-right font-mono">{fmt(p.costTotal)}</td>
                      <td className="px-4 py-2 text-xs text-wood-500 text-right">{p.pctCogs}%</td>
                      <td className="px-4 py-2 text-xs text-right">
                        {p.margin > 0 ? <span className="text-green-600">{p.margin}%</span> : <span className="text-wood-400">—</span>}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <tbody className="divide-y divide-wood-50">
              {d.operatingCosts.map((c: any) => (
                <tr key={c.concept} className="hover:bg-sand-50/50 transition-colors">
                  ...
                  <td className="py-2 text-xs text-wood-500">
                    {c.notes}
                    {c.concept === 'Hosting y servicios web' && (
                      <button onClick={() => setShowWebDetail(!showWebDetail)} className="ml-1 text-accent-gold hover:underline">
        ...
        <AnimatePresence>
          {showWebDetail && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
              <div className="mt-4 p-4 bg-sand-50 rounded-lg border border-wood-100">
                <h6 className="text-xs font-medium text-wood-700 mb-2">Desglose hosting y servicios web</h6>
                <table className="w-full text-left">
                  <thead>
                  ...
                  <tbody className="divide-y divide-wood-100">
                    {d.webServices.map((s: any) => (
                      <tr key={s.service}>
                        ...
                        <td className="py-1.5 text-[10px] text-wood-500">{s.plan}</td>
      ...
      {/* Shipping costs */}
      <Card className="overflow-hidden">
        <div className="px-5 py-3 border-b border-wood-100"><SectionTitle>Costos de Envío — {fmt(d.shippingCosts)}</SectionTitle></div>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="text-[10px] text-wood-400 uppercase tracking-wider border-b border-wood-50">
            ...
            <tbody className="divide-y divide-wood-50">
              {d.shippingCostsByCarrier.map((c: any) => (
                <tr key={c.carrier} className="hover:bg-sand-50/50 transition-colors">
                  ...
                  <td className="px-5 py-2.5 text-xs font-mono text-wood-900 text-right">{fmt(c.totalCost)}</td>
                  <td className="px-5 py-2.5 text-xs font-mono text-wood-600 text-right">{fmt(c.avgCost)}</td>
                  <td className="px-5 py-2.5 text-xs font-mono text-wood-600 text-right">{fmt(c.charged)}</td>
      ...
      {/* Discounts */}
      <Card className="overflow-hidden">
        <div className="px-5 py-3 border-b border-wood-100"><SectionTitle>Descuentos Otorgados — {fmt(d.totalDiscounts)}</SectionTitle></div>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="text-[10px] text-wood-400 uppercase tracking-wider border-b border-wood-50">
...
const InventarioTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
          ...
          <Card key={i} className="p-4">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-lg bg-accent-gold/10 flex items-center justify-center">
                <k.icon size={18} className="text-accent-gold" />
              </div>
              <div>
                <p className="text-lg font-semibold text-wood-900 font-sans">{k.value}</p>
                <p className="text-[10px] text-wood-400 uppercase tracking-wider">{k.label}</p>
                <p className="text-[10px] text-wood-500">{k.sub}</p>
              </div>
            </div>
          </Card>
        ))}
      ...
      {/* By category */}
      <Card className="overflow-hidden">
        <div className="px-5 py-3 border-b border-wood-100"><SectionTitle>Valor por categoría</SectionTitle></div>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="text-[10px] text-wood-400 uppercase tracking-wider border-b border-wood-50">
            ...
            <tbody className="divide-y divide-wood-50">
              {d.inventoryByCategory.map((c: any) => (
                <tr key={c.category} className="hover:bg-sand-50/50 transition-colors">
                  ...
                  <td className="px-5 py-2.5 text-xs text-wood-600 text-right">{c.units === 999 ? '∞' : c.units}</td>
                  <td className="px-5 py-2.5 text-xs font-mono text-wood-900 text-right">{fmt(c.costValue)}</td>
                  <td className="px-5 py-2.5 text-xs font-mono text-wood-900 text-right">{fmt(c.saleValue)}</td>
                  <td className="px-5 py-2.5 text-xs text-wood-500 text-right">{c.pct}%</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* By product */}
      <Card className="overflow-hidden">
        <div className="px-5 py-3 border-b border-wood-100"><SectionTitle>Valor por producto (detalle)</SectionTitle></div>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="text-[10px] text-wood-400 uppercase tracking-wider border-b border-wood-50">
            ...
            <tbody className="divide-y divide-wood-50">
              {d.inventoryByProduct.map((p: any) => (
                <tr key={p.product} className={`hover:bg-sand-50/50 transition-colors ${p.alert ? 'bg-red-50/30' : ''}`}>
                  ...
                  <td className="px-5 py-2.5 text-xs font-mono text-wood-600 text-right">{fmt(p.costUnit)}</td>
                  <td className="px-5 py-2.5 text-xs font-mono text-wood-900 text-right">{fmt(p.costValue)}</td>
                  <td className="px-5 py-2.5 text-xs text-right">
                    {p.rotation > 0 ? <span className={`px-1.5 py-0.5 rounded-full ${rotColor(p.rotLevel)}`}>{p.rotation}x</span> : <span className="text-wood-400">—</span>}
                  </td>
                  <td className="px-5 py-2.5 text-xs text-wood-600 text-right">{p.daysStock > 0 ? `${p.daysStock} días` : '—'}</td>
        ...
        <div className="p-4 bg-sand-50 border-t border-wood-100 space-y-1">
          <div className="flex items-center gap-4 text-[10px] text-wood-500">
      ...
      <Card className="p-5 border-amber-200 bg-amber-50/30">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h5 className="text-sm font-medium text-wood-900">Capital atrapado en inventario lento</h5>
            <p className="text-xs text-wood-600 mt-1">Productos con rotación &lt;2x/año: <span className="font-semibold">{fmt(d.stuckCapital)}</span> ({d.stuckCapitalPct}% del inventario)</p>
            <ul className="text-xs text-wood-600 mt-2 space-y-1 list-disc list-inside">
            ...
            <button onClick={() => toast.success('Redirigiendo a Ventas Flash...')} className="mt-3 inline-flex items-center gap-1.5 px-3 py-1.5 bg-accent-gold text-white text-xs rounded-lg hover:bg-accent-gold/90 transition-colors">
    ...
    <div className="space-y-6">
      ...
      <ProcessorCard title="Stripe" icon="💳" color="text-[#635BFF]">
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div><p className="text-[10px] text-wood-400 uppercase">Transacciones</p><p className="text-lg font-semibold text-wood-900 font-sans">{d.stripe.transactions}</p></div>
          <div><p className="text-[10px] text-wood-400 uppercase">Monto procesado</p><p className="text-lg font-semibold text-wood-900 font-sans">{fmt(d.stripe.processed)}</p></div>
          <div><p className="text-[10px] text-wood-400 uppercase">Comisión total</p><p className="text-lg font-semibold text-red-500 font-sans">{fmt(d.stripe.commission)} ({d.stripe.effectiveRate}%)</p></div>
          <div><p className="text-[10px] text-wood-400 uppercase">Prom. por transacción</p><p className="text-lg font-semibold text-wood-900 font-sans">{fmt(d.stripe.avgPerTransaction)}</p></div>
        </div>
        <div className="text-xs text-wood-600 space-y-1 border-t border-wood-100 pt-3">
      ...
      <ProcessorCard title="MercadoPago" icon="💙" color="text-[#009EE3]">
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div><p className="text-[10px] text-wood-400 uppercase">Transacciones</p><p className="text-lg font-semibold text-wood-900 font-sans">{d.mercadoPago.transactions}</p></div>
          <div><p className="text-[10px] text-wood-400 uppercase">Monto procesado</p><p className="text-lg font-semibold text-wood-900 font-sans">{fmt(d.mercadoPago.processed)}</p></div>
          <div><p className="text-[10px] text-wood-400 uppercase">Comisión total</p><p className="text-lg font-semibold text-red-500 font-sans">{fmt(d.mercadoPago.commission)} ({d.mercadoPago.effectiveRate}%)</p></div>
          <div><p className="text-[10px] text-wood-400 uppercase">Disponible</p><p className="text-lg font-semibold text-wood-900 font-sans">{fmt(d.mercadoPago.available)}</p></div>
        </div>
        <div className="text-xs text-wood-600 space-y-1 border-t border-wood-100 pt-3">
      ...
      {/* Bank Transfer */}
      <ProcessorCard title="Transferencias bancarias (cotizaciones)" icon="🏦" color="text-wood-900">
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div><p className="text-[10px] text-wood-400 uppercase">Transacciones</p><p className="text-lg font-semibold text-wood-900 font-sans">{d.bankTransfer.transactions}</p></div>
          <div><p className="text-[10px] text-wood-400 uppercase">Monto recibido</p><p className="text-lg font-semibold text-wood-900 font-sans">{fmt(d.bankTransfer.received)}</p></div>
          <div><p className="text-[10px] text-wood-400 uppercase">Comisión</p><p className="text-lg font-semibold text-green-600 font-sans">{fmt(0)}</p></div>
        </div>
        <div className="text-xs text-wood-600 space-y-1 border-t border-wood-100 pt-3">
      ...
      {/* Comparativa */}
      <Card className="overflow-hidden">
        <div className="px-5 py-3 border-b border-wood-100"><SectionTitle>Comparativa de comisiones</SectionTitle></div>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="text-[10px] text-wood-400 uppercase tracking-wider border-b border-wood-50">
            ...
            <tbody className="divide-y divide-wood-50">
              <tr><td className="px-5 py-2.5 text-xs text-wood-900">Stripe</td><td className="px-5 py-2.5 text-xs text-right">72%</td><td className="px-5 py-2.5 text-xs text-right">{d.stripe.effectiveRate}%</td><td className="px-5 py-2.5 text-xs font-mono text-right">$871</td><td className="px-5 py-2.5 text-xs font-mono text-right">{fmt(d.stripe.avgPerTransaction)}</td></tr>
              <tr><td className="px-5 py-2.5 text-xs text-wood-900">MercadoPago</td><td className="px-5 py-2.5 text-xs text-right">20%</td><td className="px-5 py-2.5 text-xs text-right">{d.mercadoPago.effectiveRate}%</td><td className="px-5 py-2.5 text-xs font-mono text-right">$840</td><td className="px-5 py-2.5 text-xs font-mono text-right">{fmt(d.mercadoPago.avgPerTransaction)}</td></tr>
              <tr><td className="px-5 py-2.5 text-xs text-wood-900">Transferencia</td><td className="px-5 py-2.5 text-xs text-right">8%</td><td className="px-5 py-2.5 text-xs text-right text-green-600">0.00%</td><td className="px-5 py-2.5 text-xs font-mono text-right">$1,430</td><td className="px-5 py-2.5 text-xs font-mono text-right text-green-600">$0</td></tr>
            </tbody>
          </table>
        </div>
        <div className="px-5 py-3 space-y-1.5">
          ...
          <p className="text-xs text-wood-600">Total comisiones del período: <span className="font-semibold">{fmt(d.totalCommissions)}</span> ({d.totalCommissionsPct}% de ingresos)</p>
        </div>
      </Card>
      ...
      <Card className="p-5">
        ...
        <div className="space-y-4">
              ...
              {d.pendingPayments.filter((p: any) => p.type === 'anticipo').map((p: any) => (
                <div key={p.ref} className="flex items-center justify-between p-2.5 bg-sand-50 rounded-lg text-xs">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-wood-600">{p.client}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="font-mono font-medium text-wood-900">{fmt(p.amount)}</span>
                    <span className="text-[10px] text-wood-400">{p.status} {p.date}</span>
                    <button onClick={() => toast.success(`Recordatorio enviado a ${p.client}`)} className="text-accent-gold hover:underline text-[10px]">
                      <Mail size={12} />
                    </button>
                  </div>
                </div>
          ...
          <p className="text-xs font-medium text-wood-900">Total por cobrar: <span className="text-lg font-semibold">{fmt(d.bankTransfer.totalReceivable)}</span></p>
        </div>
      </Card>
      ...
      <Card className="p-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div><p className="text-[10px] text-wood-400 uppercase">Reembolsos del período</p><p className="text-lg font-semibold text-wood-900 font-sans">{d.refunds.count} ({fmt(d.refunds.total)})</p></div>
          <div><p className="text-[10px] text-wood-400 uppercase">Tasa de reembolso</p><p className="text-lg font-semibold text-green-600 font-sans">{d.refunds.rate}% <span className="text-[10px] text-wood-400">(meta &lt;3%)</span></p></div>
          <div><p className="text-[10px] text-wood-400 uppercase">Motivos</p><p className="text-xs text-wood-600">{d.refunds.reasons.join(', ')}</p></div>
        </div>
      </Card>
    </div>
  );
...
const FlujoTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {flowKpis.map((k, i) => (
          <Card key={i} className={`p-5 ${k.highlight ? 'border-green-200 bg-green-50/30' : ''}`}>
            ...
            <p className="text-2xl font-semibold text-wood-900 font-sans">{k.value}</p>
            <p className="text-[10px] text-wood-400 uppercase tracking-wider mt-1">{k.label}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="overflow-hidden">
          <div className="px-5 py-3 border-b border-wood-100"><h5 className="text-xs font-medium text-green-600 uppercase tracking-wider">Detalle de entradas</h5></div>
          <table className="w-full text-left">
            <thead><tr className="text-[10px] text-wood-400 uppercase tracking-wider border-b border-wood-50">
        ...
        <Card className="overflow-hidden">
          <div className="px-5 py-3 border-b border-wood-100"><h5 className="text-xs font-medium text-red-500 uppercase tracking-wider">Detalle de salidas</h5></div>
          <table className="w-full text-left">
            <thead><tr className="text-[10px] text-wood-400 uppercase tracking-wider border-b border-wood-50">
          ...
          {d.upcomingPayments.map((p: any, i: any) => (
            <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-sand-50 text-xs">
              <div className="flex items-center gap-3">
                ...
                <span className="text-wood-900">{p.concept}</span>
              </div>
              <div className="flex items-center gap-3">
                <span className="font-mono font-medium text-wood-900">{fmt(p.amount)}</span>
                <span className="text-[10px] text-wood-400">{p.type}</span>
                {p.paid ? (
...
const ReportesTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
              ...
              <Card key={r.id} className="p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h5 className="text-sm font-medium text-wood-900">{r.name}</h5>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <button
                        ...
                        className="ml-auto inline-flex items-center gap-1 px-3 py-1 bg-accent-gold text-white text-[10px] rounded-lg hover:bg-accent-gold/90 transition-colors"
                      >
                        <Download size={10} /> Generar reporte
                      </button>
                    </div>
                    {r.id === 'r7' && (
                      <div className="mt-2 space-y-1 text-[10px] text-wood-500">
                        <label className="flex items-center gap-1.5"><input type="checkbox" defaultChecked className="rounded border-wood-300 text-accent-gold" /> Incluir desglose de IVA trasladado vs acreditable</label>
                        <label className="flex items-center gap-1.5"><input type="checkbox" defaultChecked className="rounded border-wood-300 text-accent-gold" /> Incluir comisiones de procesadores (gasto deducible)</label>
                        <label className="flex items-center gap-1.5"><input type="checkbox" defaultChecked className="rounded border-wood-300 text-accent-gold" /> Incluir costos de envío</label>
                      </div>
                    )}
                  </div>
                </div>
          ...
          {d.scheduledReports.map((r: any, i: any) => (
            <div key={i} className="flex items-center justify-between p-3 bg-sand-50 rounded-lg text-xs">
              <div className="flex items-center gap-3">
                ...
                <span className="text-wood-900">{r.name}</span>
                <span className="text-wood-400">{r.freq}</span>
              </div>
              {r.email && <span className="text-[10px] text-wood-500">{r.email}</span>}
            </div>
          ))}
        </div>
        <button className="mt-3 text-[10px] text-accent-gold font-medium hover:underline flex items-center gap-1">
      ...
      {/* Report history */}
      <Card className="overflow-hidden">
        <div className="px-5 py-3 border-b border-wood-100"><SectionTitle>Historial de reportes generados</SectionTitle></div>
        <table className="w-full text-left">
          <thead>
            <tr className="text-[10px] text-wood-400 uppercase tracking-wider border-b border-wood-50">
          ...
          <tbody className="divide-y divide-wood-50">
            {d.reportHistory.map((r: any, i: any) => (
              <tr key={i} className="hover:bg-sand-50/50 transition-colors">
                ...
                <td className="px-5 py-2.5 text-xs text-wood-500">{r.generated}</td>
                <td className="px-5 py-2.5"><span className="text-[10px] px-1.5 py-0.5 rounded-full bg-wood-50 text-wood-600">{r.format}</span></td>
                <td className="px-5 py-2.5 text-right">
                  <div className="flex items-center justify-end gap-1.5">
                    <button onClick={() => toast.success('Descargando...')} className="p-1 hover:bg-wood-50 rounded"><Download size={12} className="text-wood-500" /></button>
                    <button onClick={() => toast.success('Enviando por email...')} className="p-1 hover:bg-wood-50 rounded"><Mail size={12} className="text-wood-500" /></button>
                  </div>
                </td>
              </tr>
            ))}
...
const ConfigModal: React.FC<{ open: boolean; onClose: () => void }> = ({ open, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} onClick={e => e.stopPropagation()} className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[80vh] overflow-y-auto m-4">
        <div className="flex items-center justify-between px-6 py-4 border-b border-wood-100 sticky top-0 bg-white z-10">
          <h3 className="font-serif text-lg text-wood-900">Configuración Financiera</h3>
          <button onClick={onClose} className="p-1 hover:bg-wood-50 rounded"><X size={18} className="text-wood-500" /></button>
        </div>
        <div className="p-6 space-y-6">
          <div>
            <h4 className="text-xs font-medium text-wood-700 uppercase tracking-wider mb-3">Moneda</h4>
          ...
          <div>
            <h4 className="text-xs font-medium text-wood-700 uppercase tracking-wider mb-3">Impuestos</h4>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-[10px] text-wood-400">IVA (%)</label><input type="number" defaultValue={16} className="w-full border border-wood-200 rounded-lg px-3 py-2 text-xs" /></div>
              <div><label className="text-[10px] text-wood-400">IVA incluido en precios</label><div className="flex gap-3 mt-1"><label className="text-xs text-wood-600"><input type="radio" name="iva" defaultChecked className="mr-1" />Sí</label><label className="text-xs text-wood-600"><input type="radio" name="iva" className="mr-1" />No</label></div></div>
            </div>
          </div>
          <div>
            <h4 className="text-xs font-medium text-wood-700 uppercase tracking-wider mb-3">Metas financieras</h4>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-[10px] text-wood-400">Meta ingresos mensual</label><input type="number" defaultValue={150000} className="w-full border border-wood-200 rounded-lg px-3 py-2 text-xs" /></div>
              <div><label className="text-[10px] text-wood-400">Meta margen bruto (%)</label><input type="number" defaultValue={60} className="w-full border border-wood-200 rounded-lg px-3 py-2 text-xs" /></div>
              <div><label className="text-[10px] text-wood-400">Meta margen neto (%)</label><input type="number" defaultValue={25} className="w-full border border-wood-200 rounded-lg px-3 py-2 text-xs" /></div>
              <div><label className="text-[10px] text-wood-400">Meta rotación inventario</label><input type="number" defaultValue={4} className="w-full border border-wood-200 rounded-lg px-3 py-2 text-xs" /></div>
            </div>
          </div>
          <div>
            <h4 className="text-xs font-medium text-wood-700 uppercase tracking-wider mb-3">Notificaciones</h4>
            <div className="space-y-2">
              <label className="flex items-center gap-2 text-xs text-wood-600"><input type="checkbox" defaultChecked className="rounded border-wood-300 text-accent-gold" />Alertar cuando ingresos diarios caen debajo de $3,000</label>
              <label className="flex items-center gap-2 text-xs text-wood-600"><input type="checkbox" defaultChecked className="rounded border-wood-300 text-accent-gold" />Alertar cuando margen de un pedido es menor a 40%</label>
              <label className="flex items-center gap-2 text-xs text-wood-600"><input type="checkbox" defaultChecked className="rounded border-wood-300 text-accent-gold" />Alertar cuando un pago de cotización está vencido</label>
              <label className="flex items-center gap-2 text-xs text-wood-600"><input type="checkbox" defaultChecked className="rounded border-wood-300 text-accent-gold" />Resumen financiero diario a las 08:00</label>
            </div>
          </div>
        </div>
        <div className="px-6 py-4 border-t border-wood-100 sticky bottom-0 bg-white flex justify-end gap-2">
          <button onClick={onClose} className="px-4 py-2 text-xs text-wood-600 hover:bg-wood-50 rounded-lg transition-colors">Cancelar</button>
          <button onClick={() => { toast.success('Configuración guardada'); onClose(); }} className="px-4 py-2 text-xs bg-accent-gold text-white rounded-lg hover:bg-accent-gold/90 transition-colors">Guardar configuración</button>
        </div>
      </motion.div>
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <h3 className="font-serif text-lg text-wood-900 flex items-center gap-2">
          <DollarSign size={20} className="text-accent-gold" /> Finanzas
        </h3>
        <div className="flex items-center gap-2">
          <button onClick={() => toast.success('Exportando reporte...')} className="px-3 py-1.5 text-xs border border-wood-200 rounded-lg hover:bg-wood-50 transition-colors flex items-center gap-1.5">
            <Download size={12} /> Exportar
          </button>
          <button onClick={() => toast.success('Preparando impresión...')} className="px-3 py-1.5 text-xs border border-wood-200 rounded-lg hover:bg-wood-50 transition-colors flex items-center gap-1.5">
            <Printer size={12} /> Imprimir
          </button>
          <button onClick={() => setShowConfig(true)} className="px-3 py-1.5 text-xs border border-wood-200 rounded-lg hover:bg-wood-50 transition-colors flex items-center gap-1.5">
        ...
        <div className="flex gap-1 min-w-max border-b border-wood-100">
          {tabItems.map(t => (
            <button key={t.id} onClick={() => setActiveTab(t.id)}
              className={`flex items-center gap-1.5 px-3 py-2.5 text-xs transition-colors border-b-2 whitespace-nowrap ${activeTab === t.id ? 'border-accent-gold text-accent-gold font-medium' : 'border-transparent text-wood-500 hover:text-wood-700'}`}
            >
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-1.5 text-xs text-wood-600">
          ...
          <span>Período:</span>
          <select value={period} onChange={e => setPeriod(e.target.value)} className="border border-wood-200 rounded-lg px-2 py-1 text-xs bg-white">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {kpis.map((kpi, idx) => (
          <motion.div key={idx} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.06 }} className="bg-white p-4 rounded-xl border border-wood-100 shadow-sm">
            <div className="flex items-center justify-between mb-2">
            ...
            <h4 className="text-xl font-semibold text-wood-900 font-sans">{kpi.value}</h4>
            <p className="text-[10px] text-wood-400 uppercase tracking-wider mt-0.5">{kpi.label}</p>
            {kpi.note && <p className="text-[10px] text-wood-400">{kpi.note}</p>}
          </motion.div>
        ))}
      </div>
