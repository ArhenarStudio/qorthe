          ...
          <div>
            ...
            <div className="flex gap-2">
              <input value={addSku} onChange={e => setAddSku(e.target.value)} placeholder="Buscar producto..."
                ...
                className="flex-1 px-3 py-2 text-sm bg-sand-50 border border-wood-200 rounded-lg outline-none" />
              <button onClick={addItemToTransfer} className="px-3 py-2 bg-accent-gold/15 text-accent-gold text-xs rounded-lg hover:bg-accent-gold/25">
                <Plus size={14} />
              </button>
            </div>
          </div>
          ...
          {transferItems.length > 0 && (
            <div className="space-y-2">
              {transferItems.map((item, idx) => (
                <div key={idx} className="flex items-center gap-3 px-3 py-2 bg-sand-50 rounded-lg">
                  ...
                  <input type="number" min={1} value={item.quantity}
                    ...
                    className="w-16 px-2 py-1 text-xs text-center bg-white border border-wood-200 rounded" />
                  <button onClick={() => setTransferItems(prev => prev.filter((_, i) => i !== idx))}
                    className="text-wood-300 hover:text-red-500"><X size={14} /></button>
                </div>
              ))}
            </div>
          )}
                    ...
                    <div className="px-4 pb-4 bg-sand-50/50">
                      <div className="space-y-2 mb-3">
                        {(t.items || []).map((item, idx) => (
                          <div key={idx} className="flex items-center justify-between px-3 py-2 bg-white rounded-lg text-xs">
                            <div>
                      ...
                      <div className="flex gap-2">
                        {t.status === 'pending' && (
                          <>
                            ...
                            <button onClick={(e) => { e.stopPropagation(); updateTransferStatus(t.id, 'cancelled'); }}
                              className="flex items-center gap-1 px-3 py-1.5 bg-red-50 text-red-500 text-[10px] font-bold rounded-lg hover:bg-red-100">
                              <X size={12} /> Cancelar
                            </button>
                          </>
                        )}
                        {t.status === 'in_transit' && (
                          <button onClick={(e) => { e.stopPropagation(); updateTransferStatus(t.id, 'completed'); }}
                            className="flex items-center gap-1 px-3 py-1.5 bg-green-50 text-green-600 text-[10px] font-bold rounded-lg hover:bg-green-100">
