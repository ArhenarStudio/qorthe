...
export const ProductForm: React.FC<ProductFormProps> = ({ product, onBack }) => {
  ...
  const Card: React.FC<{ id: string; title: string; icon: React.ReactNode; children: React.ReactNode }> = ({ id, title, icon, children }) => (
    <div id={id} ref={el => { sectionRefs.current[id] = el; }} className="bg-white rounded-xl border border-wood-100 shadow-sm p-6 scroll-mt-4">
      <h4 className="flex items-center gap-2 font-serif text-wood-900 mb-5">
        {icon} {title}
      </h4>
      {children}
    </div>
  ...
  const InputField: React.FC<{
    ...
    <div className={className}>
      <label className="text-xs text-wood-500 mb-1.5 block">
        {label}{required && <span className="text-red-400 ml-0.5">*</span>}
      </label>
      <div className="flex items-center bg-white border border-wood-200 rounded-lg overflow-hidden focus-within:border-accent-gold/50 transition-colors">
        {prefix && <span className="pl-3 text-xs text-wood-400">{prefix}</span>}
        <input
          type={type}
          value={value}
          ...
          disabled={disabled}
          className="flex-1 px-3 py-2.5 text-sm bg-transparent outline-none text-wood-900 placeholder:text-wood-300 disabled:opacity-50"
        />
        {suffix && <span className="pr-3 text-xs text-wood-400">{suffix}</span>}
      </div>
      {hint && <p className="text-[10px] text-wood-400 mt-1 flex items-center gap-1"><Info size={10} /> {hint}</p>}
      {maxLength && <p className="text-[10px] text-wood-300 mt-0.5 text-right">{String(value).length}/{maxLength}</p>}
    </div>
  );
  ...
  const TextArea: React.FC<{
    ...
    <div>
      <label className="text-xs text-wood-500 mb-1.5 block">{label}</label>
      <textarea
        value={value}
        onChange={e => onChange(e.target.value)}
        rows={rows}
        className="w-full px-3 py-2.5 text-sm bg-white border border-wood-200 rounded-lg outline-none text-wood-900 placeholder:text-wood-300 focus:border-accent-gold/50 transition-colors resize-none"
      />
      {hint && <p className="text-[10px] text-wood-400 mt-1 flex items-center gap-1"><Info size={10} /> {hint}</p>}
      {maxLength && <p className="text-[10px] text-wood-300 mt-0.5 text-right">{value.length}/{maxLength}</p>}
    </div>
  );
  ...
  return (
    <div className="flex flex-col h-full">
      {/* ===== TOP HEADER ===== */}
      <div className="flex items-center justify-between pb-4 border-b border-wood-100 mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h3 className="font-serif text-wood-900">{isEditing ? form.name || 'Editar Producto' : 'Nuevo Producto'}</h3>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-1.5 px-3 py-2 bg-sand-50 text-wood-600 text-xs rounded-lg hover:bg-sand-100 transition-colors">
            <Eye size={13} /> Vista previa
          </button>
          <button
            onClick={() => { update('status', 'draft'); setHasChanges(false); }}
            className="flex items-center gap-1.5 px-3 py-2 bg-white border border-wood-200 text-wood-600 text-xs rounded-lg hover:bg-sand-50 transition-colors"
          >
      ...
      <div className="flex gap-6 flex-1 min-h-0">
          ...
          {sections.map(s => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs text-left transition-colors ${isActive ? 'bg-accent-gold/10 text-accent-gold' : 'text-wood-500 hover:bg-sand-50 hover:text-wood-700'}`}
              >
                <Icon size={14} />
                {s.label}
              </button>
        ...
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto space-y-5 pb-24 pr-1">
          ...
          <Card id="basic" title="Informacion Basica" icon={<FileText size={16} className="text-accent-gold" />}>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="text-xs text-wood-500 mb-2 block">Estado *</label>
                ...
                <div>
                  <label className="text-xs text-wood-500 mb-2 block">Visibilidad</label>
                  <div className="space-y-1.5">
                    {([
          ...
          <Card id="media" title="Multimedia" icon={<ImageIcon size={16} className="text-accent-gold" />}>
            <div className="space-y-4">
              <label className="text-xs text-wood-500 block">Imagenes del producto</label>
              <div className="flex flex-wrap gap-3">
                {form.images.map((img, i) => (
                  <div key={i} className="relative group w-24 h-24 rounded-xl overflow-hidden border border-wood-100">
                    <img src={img} alt="" className="w-full h-full object-cover" />
                    {i === 0 && (
                      <span className="absolute top-1 left-1 bg-accent-gold text-white text-[8px] px-1.5 py-0.5 rounded-full flex items-center gap-0.5">
                        <Star size={8} /> Principal
                      </span>
                    )}
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1">
                      {i !== 0 && (
                        <button className="p-1 bg-white rounded text-accent-gold" title="Hacer principal">
                          <Star size={12} />
                        </button>
                      )}
                      <button
                ...
                <label className="w-24 h-24 flex flex-col items-center justify-center border-2 border-dashed border-wood-200 rounded-xl cursor-pointer hover:border-accent-gold/40 transition-colors">
                  <Plus size={16} className="text-wood-300 mb-1" />
                  <span className="text-[9px] text-wood-400 text-center px-1">Agregar</span>
                  <input type="file" className="hidden" accept="image/*" />
                </label>
              </div>
              <p className="text-[10px] text-wood-400">Arrastra para reordenar. Formatos: JPG, PNG, WebP. Max 5MB. Recomendado: 1200x1200px.</p>
              ...
              <div className="bg-sand-50 rounded-xl p-4 flex items-center gap-3">
                <div className="p-2 bg-accent-gold/10 rounded-lg text-accent-gold">
                  <Sparkles size={16} />
                </div>
                <p className="text-[11px] text-wood-500">Proximamente: Sube un modelo 3D para vista AR</p>
              </div>
            </div>
          </Card>

          {/* ========== SECTION 3: Pricing ========== */}
          <Card id="pricing" title="Precios y Costos" icon={<DollarSign size={16} className="text-accent-gold" />}>
            <div className="space-y-5">
              ...
              {form.price > 0 && (
                <div className="bg-sand-50 rounded-xl p-5 space-y-3">
                  <h5 className="text-xs text-wood-700 flex items-center gap-1.5"><BarChart3 size={13} /> Analisis de Rentabilidad</h5>
                  <div className="space-y-1 text-xs">
                    <div className="flex justify-between text-wood-600">
                      <span>Precio de venta:</span>
                      <span className="text-wood-900">${form.price.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-wood-600">
                      <span>Costo del producto:</span>
                      <span className="text-red-500">-${form.cost.toLocaleString()}</span>
                    </div>
                    <div className="border-t border-wood-200 my-2" />
                    <div className="flex justify-between text-wood-900">
                      <span>Ganancia bruta:</span>
                      <span>${grossProfit.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-wood-600">Margen bruto:</span>
                      <span className={`${margin >= 50 ? 'text-green-600' : margin >= 30 ? 'text-accent-gold' : 'text-red-500'}`}>
                        {margin.toFixed(1)}% {margin >= 50 ? <Check size={10} className="inline" /> : null}
                      </span>
                    </div>
                    <div className="border-t border-wood-200 my-2" />
                    <p className="text-[10px] text-wood-400 mb-1">Estimado despues de fees:</p>
                    ...
                    <div className="mt-3 bg-accent-gold/10 rounded-lg p-3 flex items-start gap-2">
                      <Info size={12} className="text-accent-gold flex-shrink-0 mt-0.5" />
              ...
              <div>
                <label className="text-xs text-wood-500 mb-2 block">Impuestos</label>
                <div className="space-y-1.5">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-xs text-wood-700">El precio ya incluye IVA (16%)</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="tax" checked={!form.taxIncluded} onChange={() => update('taxIncluded', false)} className="accent-accent-gold" />
                    <span className="text-xs text-wood-700">Agregar IVA al precio de venta</span>
                  </label>
                </div>
                {form.taxIncluded && form.price > 0 && (
                  <p className="text-[10px] text-wood-400 mt-2">Precio con IVA desglosado: ${form.price.toLocaleString()} (IVA incluido: ${ivaAmount.toFixed(2)})</p>
                )}
              </div>
              <p className="text-[10px] text-wood-400">Moneda: MXN (Peso Mexicano) — Configuracion global</p>
            </div>
          </Card>

          {/* ========== SECTION 4: Inventory ========== */}
          <Card id="inventory" title="Inventario" icon={<Package size={16} className="text-accent-gold" />}>
            <div className="space-y-5">
              <div className="grid grid-cols-3 gap-4">
              ...
              <p className="text-[10px] text-wood-400 flex items-center gap-1"><Info size={10} /> Recibiras alerta cuando el stock llegue al punto de reorden.</p>

              <div>
                <label className="text-xs text-wood-500 mb-2 block">Opciones de inventario</label>
                <div className="space-y-1.5">
                  {([
              ...
              {isEditing && (
                <div className="bg-sand-50 rounded-xl p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h5 className="text-xs text-wood-700 flex items-center gap-1.5"><BarChart3 size={12} /> Historial de Stock</h5>
                  ...
                  <div className="space-y-2">
                    {stockHistory.length === 0 ? (
                      ...
                      <div key={i} className="flex items-center gap-3 text-[11px]">
                        ...
                        <span className="text-wood-400">{entry.reason}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {/* Manual adjust */}
              <div>
                <label className="text-xs text-wood-500 mb-2 block">Ajustar stock manualmente</label>
                <div className="flex gap-3 items-end">
                  <div className="w-24">
                    <label className="text-[10px] text-wood-400 block mb-1">Cantidad</label>
                    <input
                      type="number"
                      value={stockAdjust.qty}
                      onChange={e => setStockAdjust(prev => ({ ...prev, qty: Number(e.target.value) }))}
                      className="w-full px-3 py-2 text-sm border border-wood-200 rounded-lg outline-none focus:border-accent-gold/50"
                    />
                  </div>
                  <div className="flex-1">
                    <label className="text-[10px] text-wood-400 block mb-1">Motivo</label>
                    <select
                      value={stockAdjust.reason}
                      onChange={e => setStockAdjust(prev => ({ ...prev, reason: e.target.value }))}
                      className="w-full px-3 py-2 text-xs border border-wood-200 rounded-lg outline-none focus:border-accent-gold/50 bg-white text-wood-700"
                    >
          ...
          {/* ========== SECTION 5: Shipping ========== */}
          <Card id="shipping" title="Envio y Dimensiones Fisicas" icon={<Truck size={16} className="text-accent-gold" />}>
            <div className="space-y-5">
              <div className="space-y-1.5">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-xs text-wood-700">Este producto requiere envio</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-xs text-wood-700">Producto digital (no requiere envio)</span>
                </label>
              </div>
              ...
              {form.requiresShipping && (
                <>
                  <div>
                    <label className="text-xs text-wood-500 mb-2 block">Dimensiones del Producto</label>
                  ...
                  <div>
                    <label className="text-xs text-wood-500 mb-2 block">Dimensiones de Empaque</label>
                    <div className="grid grid-cols-4 gap-3">
                    ...
                    <p className="text-[10px] text-wood-400 mt-1 flex items-center gap-1"><Info size={10} /> Peso total = peso del producto + peso del empaque (0.5 kg).</p>
                  </div>

                  <div>
                    <label className="text-xs text-wood-500 mb-2 block">Perfil de envio</label>
                    <select
                      value={form.shippingProfile}
                      onChange={e => update('shippingProfile', e.target.value)}
                      className="w-full max-w-xs px-3 py-2.5 text-xs border border-wood-200 rounded-lg bg-white text-wood-700 outline-none focus:border-accent-gold/50"
                    >
                  ...
                  <div className="bg-sand-50 rounded-xl p-4">
                    <h5 className="text-xs text-wood-700 mb-3">Preview de costos de envio</h5>
                    <div className="space-y-1.5 text-[11px]">
                      {[
                        ...
                        <div key={s.carrier} className="flex justify-between">
                          <span className="text-wood-600">{s.carrier}:</span>
                          <span className="text-wood-900">{s.price} <span className="text-wood-400">({s.time})</span></span>
                        </div>
                      ))}
                    </div>
                    <p className="text-[10px] text-wood-400 mt-2 flex items-center gap-1"><Info size={10} /> Estos son estimados. El precio real depende del destino.</p>
                  </div>

                  {/* Customs */}
                  <div className="opacity-60">
                    <label className="text-xs text-wood-400 mb-2 block">Informacion aduanal (envios internacionales — futuro)</label>
          ...
          {/* ========== SECTION 6: Variants ========== */}
          <Card id="variants" title="Variantes" icon={<Layers size={16} className="text-accent-gold" />}>
            <div className="space-y-5">
              <div>
                <label className="text-xs text-wood-500 mb-2 block">Opcion del producto</label>
                <div className="flex items-center gap-3">
                  <select
                    value={form.variantOption}
                    onChange={e => update('variantOption', e.target.value)}
                    className="px-3 py-2 text-xs border border-wood-200 rounded-lg bg-white text-wood-700 outline-none"
                  >
                    {['Tamano', 'Color', 'Acabado', 'Estilo'].map(o => <option key={o}>{o}</option>)}
                  </select>
                  <div className="flex flex-wrap gap-1.5">
                    {form.variants.map(v => (
                      <span key={v.id} className="inline-flex items-center gap-1 px-2 py-1 bg-sand-100 text-wood-700 text-[11px] rounded-full">
                        {v.name}
                        <button onClick={() => update('variants', form.variants.filter(x => x.id !== v.id))} className="text-wood-400 hover:text-red-500">
                          <X size={10} />
                        </button>
                      </span>
                    ))}
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-xs text-accent-gold hover:underline whitespace-nowrap"
                  >
                        ...
                        {form.variants.map(v => (
                          <tr key={v.id} className="hover:bg-sand-50/30">
                            <td className="px-4 py-2">
                              <input value={v.name} onChange={e => update('variants', form.variants.map(x => x.id === v.id ? { ...x, name: e.target.value } : x))} className="text-xs text-wood-900 bg-transparent outline-none w-24" />
                            </td>
                            <td className="px-4 py-2">
                              <input value={v.sku} onChange={e => update('variants', form.variants.map(x => x.id === v.id ? { ...x, sku: e.target.value } : x))} className="text-xs text-wood-500 font-mono bg-transparent outline-none w-36" />
                            </td>
                            <td className="px-4 py-2">
                              <input type="number" value={v.price} onChange={e => update('variants', form.variants.map(x => x.id === v.id ? { ...x, price: Number(e.target.value) } : x))} className="text-xs text-wood-900 bg-transparent outline-none w-20" />
                            </td>
                            <td className="px-4 py-2">
                              <input type="number" value={v.cost} onChange={e => update('variants', form.variants.map(x => x.id === v.id ? { ...x, cost: Number(e.target.value) } : x))} className="text-xs text-wood-900 bg-transparent outline-none w-20" />
                            </td>
                            <td className="px-4 py-2">
                              <input type="number" value={v.stock} onChange={e => update('variants', form.variants.map(x => x.id === v.id ? { ...x, stock: Number(e.target.value) } : x))} className="text-xs text-wood-900 bg-transparent outline-none w-16" />
                            </td>
                            <td className="px-4 py-2">
                              <button onClick={() => update('variants', form.variants.filter(x => x.id !== v.id))} className="p-1 text-wood-300 hover:text-red-500"><Trash2 size={12} /></button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                  ...
                  <div className="overflow-x-auto rounded-xl border border-wood-100">
                    <table className="w-full text-left">
                      ...
                      <tbody className="divide-y divide-wood-50">
                        {form.variants.map(v => (
                          <tr key={v.id} className="hover:bg-sand-50/30">
                            <td className="px-4 py-2 text-xs text-wood-700">{v.name}</td>
                            <td className="px-4 py-2">
                              <input value={v.dimensions.split('×')[0] || ''} className="text-xs text-wood-600 bg-transparent outline-none w-16" readOnly />
                            </td>
                            <td className="px-4 py-2">
                              <input value={v.dimensions.split('×')[1] || ''} className="text-xs text-wood-600 bg-transparent outline-none w-16" readOnly />
                            </td>
                            <td className="px-4 py-2">
                              <input value={v.dimensions.split('×')[2] || ''} className="text-xs text-wood-600 bg-transparent outline-none w-16" readOnly />
                            </td>
                            <td className="px-4 py-2 text-xs text-wood-600">{v.weight} kg</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  </div>
                  <p className="text-[10px] text-wood-400 flex items-center gap-1"><Info size={10} /> Las dimensiones por variante se usan para cotizacion de envio.</p>
                </>
              )}
            </div>
          </Card>
          ...
          <Card id="customization" title="Personalizacion (Grabado Laser y Artesania)" icon={<Scissors size={16} className="text-accent-gold" />}>
            <div className="space-y-5">
              {/* Laser */}
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-xs text-wood-700">Este producto acepta grabado laser</span>
              </label>

              {form.laserEngraving && (
                <>
                  <div>
                    <label className="text-xs text-wood-500 mb-2 block">Area maxima de grabado</label>
                  ...
                  <div className="bg-sand-50 rounded-xl p-4">
                    <p className="text-[10px] text-wood-500 mb-2">Preview del area de grabado:</p>
                    <div className="relative mx-auto" style={{ width: 200, height: 120 }}>
                      <div className="absolute inset-0 bg-wood-200/40 rounded-xl border-2 border-wood-300 flex items-center justify-center">
                        <span className="text-[9px] text-wood-400 absolute bottom-1 right-2">
                          {form.productLength || '45'} x {form.productWidth || '27'} cm
                        </span>
                        <div className="bg-accent-gold/15 border-2 border-dashed border-accent-gold/40 rounded-lg px-4 py-3 text-center">
                          <span className="text-[10px] text-accent-gold">AREA GRABADO</span>
                          <br />
                          <span className="text-[9px] text-accent-gold/70">{form.engravingMaxWidth} x {form.engravingMaxHeight} cm</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    <label className="text-xs text-wood-500 mb-2 block">Posiciones disponibles</label>
                    <div className="space-y-1.5">
                      {['Centro', 'Inferior derecha', 'Inferior izquierda', 'Personalizada (el cliente elige libremente)'].map(pos => (
                        <label key={pos} className="flex items-center gap-2 cursor-pointer">
                          ...
                          />
                          <span className="text-xs text-wood-700">{pos}</span>
                        </label>
                      ))}
                    </div>
                  </div>
              ...
              <div className="border-t border-wood-100 pt-5">
                <h5 className="text-xs text-wood-700 mb-3">Materiales y Artesania</h5>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs text-wood-500 mb-1.5 block">Tipo de madera *</label>
                    <select value={form.woodType} onChange={e => update('woodType', e.target.value)} className="w-full px-3 py-2.5 text-xs border border-wood-200 rounded-lg bg-white text-wood-700 outline-none">
                      {['Parota', 'Cedro Rojo', 'Rosa Morada', 'Tzalam', 'Nogal', 'Mezquite', 'Pino', 'Teca', 'Otro'].map(w => <option key={w}>{w}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="text-xs text-wood-500 mb-1.5 block">Acabado *</label>
                    <select value={form.finish} onChange={e => update('finish', e.target.value)} className="w-full px-3 py-2.5 text-xs border border-wood-200 rounded-lg bg-white text-wood-700 outline-none">
                ...
                <div className="grid grid-cols-2 gap-4 mt-4">
                  ...
                  <div>
                    <label className="text-xs text-wood-500 mb-2 block">Tiempo de produccion</label>
                    <div className="space-y-1.5">
                      <label className="flex items-center gap-2 cursor-pointer">
                        ...
                        <span className="text-xs text-wood-700">En stock — Envio inmediato</span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        ...
                        <span className="text-xs text-wood-700">Fabricacion sobre pedido: {form.productionDays} dias</span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        ...
                        <span className="text-xs text-wood-700">Temporalmente no disponible</span>
                      </label>
                    </div>
                  </div>
                </div>
          ...
          <Card id="categories" title="Organizacion" icon={<FolderTree size={16} className="text-accent-gold" />}>
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs text-wood-500 mb-1.5 block">Categoria principal *</label>
                  <select value={form.mainCategory} onChange={e => update('mainCategory', e.target.value)} className="w-full px-3 py-2.5 text-xs border border-wood-200 rounded-lg bg-white text-wood-700 outline-none">
                    <option value="">Seleccionar...</option>
                    {['Tablas para Cortar', 'Sets y Colecciones', 'Accesorios', 'Servicios'].map(c => <option key={c}>{c}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-xs text-wood-500 mb-1.5 block">Sub-categoria</label>
                  <select value={form.subCategory} onChange={e => update('subCategory', e.target.value)} className="w-full px-3 py-2.5 text-xs border border-wood-200 rounded-lg bg-white text-wood-700 outline-none">
                    <option value="">Seleccionar...</option>
              ...
              {/* Category tree */}
              <div className="bg-sand-50 rounded-xl p-4 text-xs text-wood-600">
                <p className="text-[10px] text-wood-400 mb-2">Arbol de categorias:</p>
                ...
                <button className="flex items-center gap-1 text-accent-gold text-[11px] mt-3 hover:underline">
              ...
              {/* Tags */}
              <div>
                <label className="text-xs text-wood-500 mb-1.5 block">Tags / Etiquetas</label>
                <div className="flex flex-wrap items-center gap-1.5 p-2.5 border border-wood-200 rounded-lg bg-white min-h-[40px]">
                  {form.tags.map(tag => (
                    <span key={tag} className="inline-flex items-center gap-1 px-2 py-0.5 bg-sand-100 text-wood-700 text-[11px] rounded-full">
                      {tag}
                      <button onClick={() => update('tags', form.tags.filter(t => t !== tag))} className="text-wood-400 hover:text-red-500"><X size={10} /></button>
                    </span>
                  ))}
                  <input
                    value={newTag}
                    ...
                    className="text-xs bg-transparent outline-none text-wood-700 placeholder:text-wood-300 min-w-[80px] flex-1"
                  />
                </div>
                <p className="text-[10px] text-wood-400 mt-1">Autocomplete con tags existentes. Enter para crear nuevo.</p>
              </div>

              {/* Collections */}
              <div>
                <label className="text-xs text-wood-500 mb-2 block">Colecciones</label>
                <div className="space-y-1.5">
                  {['Best Sellers', 'Navidad 2026', 'Nuevos Productos', 'Ofertas Especiales'].map(col => (
                    <label key={col} className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        className="accent-accent-gold rounded"
                      />
                      <span className="text-xs text-wood-700">{col}</span>
                    </label>
                  ))}
                </div>
                <button className="flex items-center gap-1 text-accent-gold text-[11px] mt-2 hover:underline">
                  <Plus size={10} /> Nueva coleccion
                </button>
              </div>

              <div>
                <label className="text-xs text-wood-500 mb-1.5 block">Tipo de producto</label>
                <select value={form.productType} onChange={e => update('productType', e.target.value)} className="w-full max-w-xs px-3 py-2.5 text-xs border border-wood-200 rounded-lg bg-white text-wood-700 outline-none">
                  <option value="">Seleccionar...</option>
                  {['Tabla para cortar', 'Set de tablas', 'Accesorio', 'Servicio'].map(t => <option key={t}>{t}</option>)}
                </select>
                <p className="text-[10px] text-wood-400 mt-1">Usado para filtros y organizacion interna.</p>
              </div>
            </div>
          </Card>

          {/* ========== SECTION 9: SEO ========== */}
          <Card id="seo" title="SEO — Optimizacion para Buscadores" icon={<SearchIcon size={16} className="text-accent-gold" />}>
            <div className="space-y-5">
              {/* Google preview */}
              <div className="bg-sand-50 rounded-xl p-4">
                ...
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-wood-500">{form.metaDescription || form.shortDescription || 'Descripcion del producto...'}</p>
                </div>
              </div>
              ...
              <div>
                <label className="text-xs text-wood-500 mb-1.5 block">URL amigable (slug)</label>
                <div className="flex items-center text-xs text-wood-400">
                  <span>davidsonsdesign.com/shop/</span>
                  <input
                    value={form.slug}
                    onChange={e => update('slug', e.target.value)}
                    className="ml-1 px-2 py-1.5 border border-wood-200 rounded-lg text-wood-700 outline-none focus:border-accent-gold/50 bg-white"
                  />
                </div>
                <p className="text-[10px] text-wood-400 mt-1 flex items-center gap-1"><Info size={10} /> Se genera automaticamente del nombre. Editable.</p>
              </div>

              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={form.autoSeo} onChange={() => update('autoSeo', !form.autoSeo)} className="accent-accent-gold rounded" />
                <span className="text-xs text-wood-700">Generar SEO automaticamente del nombre y descripcion</span>
              </label>

              <button className="flex items-center gap-1.5 px-4 py-2 bg-accent-gold/10 text-accent-gold text-xs rounded-lg hover:bg-accent-gold/20 transition-colors">
          ...
          {/* ========== SECTION 10: Config ========== */}
          <Card id="config" title="Configuracion Avanzada" icon={<Settings size={16} className="text-accent-gold" />}>
            <div className="space-y-5">
              {/* Related */}
              <div>
                <label className="text-xs text-wood-500 mb-1.5 block">Producto relacionado con</label>
                <div className="flex flex-wrap items-center gap-1.5 p-2.5 border border-wood-200 rounded-lg bg-white min-h-[40px]">
                  {form.relatedProducts.map(p => (
                    <span key={p} className="inline-flex items-center gap-1 px-2 py-0.5 bg-sand-100 text-wood-700 text-[11px] rounded-full">
                      {p}
                      <button onClick={() => update('relatedProducts', form.relatedProducts.filter(x => x !== p))} className="text-wood-400 hover:text-red-500"><X size={10} /></button>
                    </span>
                  ))}
                  <span className="text-[11px] text-wood-300 cursor-pointer">+ buscar...</span>
                </div>
                <p className="text-[10px] text-wood-400 mt-1">Aparecen en "Tambien te puede interesar" en la tienda.</p>
              </div>

              {/* Up-sell */}
              <div>
                <label className="text-xs text-wood-500 mb-1.5 block">Up-sell (compra mayor)</label>
                <div className="flex flex-wrap items-center gap-1.5 p-2.5 border border-wood-200 rounded-lg bg-white min-h-[40px]">
                  <span className="text-[11px] text-wood-300 cursor-pointer">+ buscar...</span>
                </div>
                <p className="text-[10px] text-wood-400 mt-1">Aparece como "Quieres el set completo?" en la tienda.</p>
              </div>

              {/* Cross-sell */}
              <div>
                <label className="text-xs text-wood-500 mb-1.5 block">Cross-sell (complementarios)</label>
                <div className="flex flex-wrap items-center gap-1.5 p-2.5 border border-wood-200 rounded-lg bg-white min-h-[40px]">
                  <span className="text-[11px] text-wood-300 cursor-pointer">+ buscar...</span>
                </div>
                <p className="text-[10px] text-wood-400 mt-1">Aparece en el carrito como "Complementa tu compra".</p>
              </div>
              ...
              {/* Dev flags */}
              <div className="border-t border-wood-100 pt-4 opacity-60">
                <p className="text-[10px] text-wood-400 mb-2">Producto SaaS flags (solo visible para Owner/Dev)</p>
                <div className="space-y-1.5">
                  {['Ocultar del catalogo publico', 'No rastrear inventario', 'Producto de servicio (no requiere envio)'].map(f => (
                    <label key={f} className="flex items-center gap-2 cursor-pointer">
                      ...
                      <span className="text-xs text-wood-500">{f}</span>
                    </label>
                  ))}
                </div>
                {isEditing && <p className="text-[10px] text-wood-300 mt-2">Medusa ID: prod_01KJ4E6ZFNMMVE9SQES7TJDH1J</p>}
              </div>
            </div>
          </Card>
          ...
          {isEditing && product && (
            <Card id="stats" title="Estadisticas" icon={<BarChart3 size={16} className="text-accent-gold" />}>
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[
                    ...
                    <div key={s.label} className="bg-sand-50 rounded-xl p-4 text-center">
                      ...
                      <p className="text-[10px] text-wood-500 mt-0.5">{s.label}</p>
                      <p className="text-[10px] text-green-600 mt-1">{s.change} vs prev</p>
                    </div>
                  ))}
                </div>

                <p className="text-xs text-wood-600">Tasa de conversion: <span className="text-wood-900">3.4%</span> (visitas → compra)</p>
                ...
                {form.variants.length > 0 && (
                  <div>
                    <p className="text-xs text-wood-700 mb-2">Variantes mas vendidas:</p>
                    <div className="space-y-1.5">
                      {[
                        ...
                        <div key={v.name} className="flex items-center gap-3 text-xs">
                          <span className="text-wood-400 w-4">{i + 1}.</span>
                          <span className="text-wood-700 w-20">{v.name}</span>
                          <div className="flex-1 h-2 bg-wood-100 rounded-full overflow-hidden">
                            <div className="h-full bg-accent-gold rounded-full" style={{ width: `${v.pct}%` }} />
                          </div>
                          <span className="text-wood-500 w-20 text-right">{v.qty} uds ({v.pct}%)</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div>
                  <p className="text-xs text-wood-700 mb-2">Frecuentemente comprado con:</p>
      ...
      {/* ===== STICKY FOOTER ===== */}
      {hasChanges && (
        <div className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-wood-200 shadow-lg px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-4 py-2 text-xs text-wood-600 hover:text-wood-800 transition-colors"
            >
              Descartar cambios
            </button>
            {isEditing ? (
              ...
              <>
                <button
                  onClick={() => setHasChanges(false)}
                  className="px-4 py-2 bg-white border border-wood-200 text-wood-600 text-xs rounded-lg hover:bg-sand-50 transition-colors"
                >
                  Guardar borrador
                </button>
                <button
