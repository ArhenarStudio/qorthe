...
export const ProductForm: React.FC<ProductFormProps> = ({ product, onBack }) => {
    ...
    <div className={className}>
      ...
      <div className="flex items-center bg-[var(--surface)] border border-[var(--border)] rounded-[var(--radius-card)] overflow-hidden focus-within:border-[var(--accent)]/50 transition-colors">
        ...
        <input
          ...
          disabled={disabled}
          className="flex-1 px-3 py-2.5 text-sm bg-transparent outline-none text-[var(--text)] placeholder:text-[var(--text-muted)] disabled:opacity-50"
        />
      ...
      {maxLength && <p className="text-[10px] text-[var(--text-muted)] mt-0.5 text-right">{String(value).length}/{maxLength}</p>}
    </div>
  );
  ...
  const TextArea: React.FC<{
    ...
    <div>
      <label className="text-xs text-[var(--text-secondary)] mb-1.5 block">{label}</label>
      <textarea
        value={value}
        onChange={e => onChange(e.target.value)}
        rows={rows}
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-[var(--border)] mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] text-[var(--text-muted)] uppercase tracking-wider">Productos</p>
            <h3 className="font-serif text-[var(--text)]">{isEditing ? form.name || 'Editar Producto' : 'Nuevo Producto'}</h3>
          </div>
        </div>
      ...
      <div className="flex gap-6 flex-1 min-h-0">
        ...
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto space-y-5 pb-24 pr-1">
          ...
          <Card id="basic" title="Informacion Basica" icon={<FileText size={16} className="text-[var(--accent)]" />}>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="text-xs text-[var(--text-secondary)] mb-2 block">Estado *</label>
                ...
                <div>
                  <label className="text-xs text-[var(--text-secondary)] mb-2 block">Visibilidad</label>
                  <div className="space-y-1.5">
                    {([
                      ...
                      <label key={opt.key} className="flex items-center gap-2 cursor-pointer">
                        ...
                        <span className="text-xs text-[var(--text)]">{opt.label}</span>
                      </label>
                    ))}
                  </div>
                </div>
          ...
          <Card id="media" title="Multimedia" icon={<ImageIcon size={16} className="text-[var(--accent)]" />}>
            <div className="space-y-4">
                ...
                {form.images.map((img, i) => (
                  <div key={i} className="relative group w-24 h-24 rounded-[var(--radius-card)] overflow-hidden border border-[var(--border)]">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1">
                      {i !== 0 && (
                        <button className="p-1 bg-[var(--surface)] rounded text-[var(--accent)]" title="Hacer principal">
                          <Star size={12} />
                        </button>
                      )}
                      <button
              ...
              <p className="text-[10px] text-[var(--text-muted)]">Arrastra para reordenar. Formatos: JPG, PNG, WebP. Max 5MB. Recomendado: 1200x1200px.</p>
          ...
          {/* ========== SECTION 3: Pricing ========== */}
          <Card id="pricing" title="Precios y Costos" icon={<DollarSign size={16} className="text-[var(--accent)]" />}>
            <div className="space-y-5">
                ...
                <div className="bg-[var(--surface2)] rounded-[var(--radius-card)] p-5 space-y-3">
                  ...
                  <div className="space-y-1 text-xs">
                    <div className="flex justify-between text-[var(--text-secondary)]">
                      ...
                      <span className="text-[var(--text)]">${form.price.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-[var(--text-secondary)]">
                    ...
                    <div className="border-t border-[var(--border)] my-2" />
                    <p className="text-[10px] text-[var(--text-muted)] mb-1">Estimado despues de fees:</p>
                    <div className="flex justify-between text-[var(--text-secondary)]">
              ...
              <div>
                <label className="text-xs text-[var(--text-secondary)] mb-2 block">Impuestos</label>
                <div className="space-y-1.5">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-xs text-[var(--text)]">El precio ya incluye IVA (16%)</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="tax" checked={!form.taxIncluded} onChange={() => update('taxIncluded', false)} className="accent-accent-gold" />
                    <span className="text-xs text-[var(--text)]">Agregar IVA al precio de venta</span>
                  </label>
                </div>
                {form.taxIncluded && form.price > 0 && (
                  <p className="text-[10px] text-[var(--text-muted)] mt-2">Precio con IVA desglosado: ${form.price.toLocaleString()} (IVA incluido: ${ivaAmount.toFixed(2)})</p>
                )}
              </div>
              <p className="text-[10px] text-[var(--text-muted)]">Moneda: MXN (Peso Mexicano) — Configuracion global</p>
            </div>
          </Card>

          {/* ========== SECTION 4: Inventory ========== */}
          <Card id="inventory" title="Inventario" icon={<Package size={16} className="text-[var(--accent)]" />}>
            <div className="space-y-5">
              <div className="grid grid-cols-3 gap-4">
              ...
              <p className="text-[10px] text-[var(--text-muted)] flex items-center gap-1"><Info size={10} /> Recibiras alerta cuando el stock llegue al punto de reorden.</p>

              <div>
                <label className="text-xs text-[var(--text-secondary)] mb-2 block">Opciones de inventario</label>
                <div className="space-y-1.5">
                  {([
                ...
                <div className="bg-[var(--surface2)] rounded-[var(--radius-card)] p-4">
                  ...
                  <div className="space-y-2">
                    {stockHistory.length === 0 ? (
                      <p className="text-[10px] text-[var(--text-muted)]">Sin movimientos registrados</p>
              ...
              {/* Manual adjust */}
              <div>
                <label className="text-xs text-[var(--text-secondary)] mb-2 block">Ajustar stock manualmente</label>
                <div className="flex gap-3 items-end">
                  <div className="w-24">
                    <label className="text-[10px] text-[var(--text-muted)] block mb-1">Cantidad</label>
                    <input
                      type="number"
                  ...
                  <div className="flex-1">
                    <label className="text-[10px] text-[var(--text-muted)] block mb-1">Motivo</label>
                    <select
          ...
          {/* ========== SECTION 5: Shipping ========== */}
          <Card id="shipping" title="Envio y Dimensiones Fisicas" icon={<Truck size={16} className="text-[var(--accent)]" />}>
            <div className="space-y-5">
              <div className="space-y-1.5">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-xs text-[var(--text)]">Este producto requiere envio</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-xs text-[var(--text)]">Producto digital (no requiere envio)</span>
                </label>
              </div>
              ...
              {form.requiresShipping && (
                <>
                  <div>
                    <label className="text-xs text-[var(--text-secondary)] mb-2 block">Dimensiones del Producto</label>
                  ...
                  <div>
                    <label className="text-xs text-[var(--text-secondary)] mb-2 block">Dimensiones de Empaque</label>
                    <div className="grid grid-cols-4 gap-3">
                    ...
                    <p className="text-[10px] text-[var(--text-muted)] mt-1 flex items-center gap-1"><Info size={10} /> Peso total = peso del producto + peso del empaque (0.5 kg).</p>
                  </div>

                  <div>
                    <label className="text-xs text-[var(--text-secondary)] mb-2 block">Perfil de envio</label>
                    <select
                    ...
                    <div className="space-y-1.5 text-[11px]">
                      {[
                        ...
                        <div key={s.carrier} className="flex justify-between">
                          <span className="text-[var(--text-secondary)]">{s.carrier}:</span>
                          <span className="text-[var(--text)]">{s.price} <span className="text-[var(--text-muted)]">({s.time})</span></span>
                        </div>
                      ))}
                    </div>
                    <p className="text-[10px] text-[var(--text-muted)] mt-2 flex items-center gap-1"><Info size={10} /> Estos son estimados. El precio real depende del destino.</p>
                  </div>

                  {/* Customs */}
                  <div className="opacity-60">
                    <label className="text-xs text-[var(--text-muted)] mb-2 block">Informacion aduanal (envios internacionales — futuro)</label>
          ...
          {/* ========== SECTION 6: Variants ========== */}
          <Card id="variants" title="Variantes" icon={<Layers size={16} className="text-[var(--accent)]" />}>
            <div className="space-y-5">
              <div>
                <label className="text-xs text-[var(--text-secondary)] mb-2 block">Opcion del producto</label>
                <div className="flex items-center gap-3">
                  <select
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-xs text-[var(--accent)] hover:underline whitespace-nowrap"
                  >
                  ...
                  <div className="overflow-x-auto rounded-[var(--radius-card)] border border-[var(--border)]">
                    <table className="w-full text-left">
                      ...
                      <tbody className="divide-y divide-wood-50">
                        {form.variants.map(v => (
                          <tr key={v.id} className="hover:bg-[var(--surface2)]/30">
                            <td className="px-4 py-2">
                              <input value={v.name} onChange={e => update('variants', form.variants.map(x => x.id === v.id ? { ...x, name: e.target.value } : x))} className="text-xs text-[var(--text)] bg-transparent outline-none w-24" />
                            </td>
                            <td className="px-4 py-2">
                              <input value={v.sku} onChange={e => update('variants', form.variants.map(x => x.id === v.id ? { ...x, sku: e.target.value } : x))} className="text-xs text-[var(--text-secondary)] font-mono bg-transparent outline-none w-36" />
                            </td>
                            <td className="px-4 py-2">
                              <input type="number" value={v.price} onChange={e => update('variants', form.variants.map(x => x.id === v.id ? { ...x, price: Number(e.target.value) } : x))} className="text-xs text-[var(--text)] bg-transparent outline-none w-20" />
                            </td>
                            <td className="px-4 py-2">
                              <input type="number" value={v.cost} onChange={e => update('variants', form.variants.map(x => x.id === v.id ? { ...x, cost: Number(e.target.value) } : x))} className="text-xs text-[var(--text)] bg-transparent outline-none w-20" />
                            </td>
                            <td className="px-4 py-2">
                  ...
                  <div className="overflow-x-auto rounded-[var(--radius-card)] border border-[var(--border)]">
                    <table className="w-full text-left">
                      ...
                      <tbody className="divide-y divide-wood-50">
                        {form.variants.map(v => (
                          <tr key={v.id} className="hover:bg-[var(--surface2)]/30">
                            <td className="px-4 py-2 text-xs text-[var(--text)]">{v.name}</td>
                            <td className="px-4 py-2">
                              <input value={v.dimensions.split('×')[0] || ''} className="text-xs text-[var(--text-secondary)] bg-transparent outline-none w-16" readOnly />
                            </td>
                            <td className="px-4 py-2">
                              <input value={v.dimensions.split('×')[1] || ''} className="text-xs text-[var(--text-secondary)] bg-transparent outline-none w-16" readOnly />
                            </td>
                            <td className="px-4 py-2">
                              <input value={v.dimensions.split('×')[2] || ''} className="text-xs text-[var(--text-secondary)] bg-transparent outline-none w-16" readOnly />
                            </td>
                            <td className="px-4 py-2 text-xs text-[var(--text-secondary)]">{v.weight} kg</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  </div>
                  <p className="text-[10px] text-[var(--text-muted)] flex items-center gap-1"><Info size={10} /> Las dimensiones por variante se usan para cotizacion de envio.</p>
                </>
              )}
            </div>
          </Card>
          ...
          <Card id="customization" title="Personalizacion (Grabado Laser y Artesania)" icon={<Scissors size={16} className="text-[var(--accent)]" />}>
            <div className="space-y-5">
              {/* Laser */}
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-xs text-[var(--text)]">Este producto acepta grabado laser</span>
              </label>

              {form.laserEngraving && (
                <>
                  <div>
                    <label className="text-xs text-[var(--text-secondary)] mb-2 block">Area maxima de grabado</label>
                  ...
                  <div>
                    <label className="text-xs text-[var(--text-secondary)] mb-2 block">Posiciones disponibles</label>
                    <div className="space-y-1.5">
                      {['Centro', 'Inferior derecha', 'Inferior izquierda', 'Personalizada (el cliente elige libremente)'].map(pos => (
                        <label key={pos} className="flex items-center gap-2 cursor-pointer">
                          ...
                          />
                          <span className="text-xs text-[var(--text)]">{pos}</span>
                        </label>
                      ))}
                    </div>
                  </div>
              ...
              <div className="border-t border-[var(--border)] pt-5">
                <h5 className="text-xs text-[var(--text)] mb-3">Materiales y Artesania</h5>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                ...
                <div className="grid grid-cols-2 gap-4 mt-4">
                  ...
                  <div>
                    <label className="text-xs text-[var(--text-secondary)] mb-2 block">Tiempo de produccion</label>
                    <div className="space-y-1.5">
                      <label className="flex items-center gap-2 cursor-pointer">
                        ...
                        <span className="text-xs text-[var(--text)]">En stock — Envio inmediato</span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        ...
                        <span className="text-xs text-[var(--text)]">Fabricacion sobre pedido: {form.productionDays} dias</span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        ...
                        <span className="text-xs text-[var(--text)]">Temporalmente no disponible</span>
                      </label>
                    </div>
                  </div>
                </div>
          ...
          <Card id="categories" title="Organizacion" icon={<FolderTree size={16} className="text-[var(--accent)]" />}>
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
              ...
              <div className="bg-[var(--surface2)] rounded-[var(--radius-card)] p-4 text-xs text-[var(--text-secondary)]">
                ...
                <button className="flex items-center gap-1 text-[var(--accent)] text-[11px] mt-3 hover:underline">
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-1.5 p-2.5 border border-[var(--border)] rounded-[var(--radius-card)] bg-[var(--surface)] min-h-[40px]">
                  ...
                  <input
                    ...
                    className="text-xs bg-transparent outline-none text-[var(--text)] placeholder:text-[var(--text-muted)] min-w-[80px] flex-1"
                  />
                </div>
                <p className="text-[10px] text-[var(--text-muted)] mt-1">Autocomplete con tags existentes. Enter para crear nuevo.</p>
              </div>

              {/* Collections */}
              <div>
                <label className="text-xs text-[var(--text-secondary)] mb-2 block">Colecciones</label>
                <div className="space-y-1.5">
                  {['Best Sellers', 'Navidad 2026', 'Nuevos Productos', 'Ofertas Especiales'].map(col => (
                    <label key={col} className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        className="accent-accent-gold rounded"
                      />
                      <span className="text-xs text-[var(--text)]">{col}</span>
                    </label>
                  ))}
                </div>
                <button className="flex items-center gap-1 text-[var(--accent)] text-[11px] mt-2 hover:underline">
              ...
              <div>
                ...
                <p className="text-[10px] text-[var(--text-muted)] mt-1">Usado para filtros y organizacion interna.</p>
              </div>
            </div>
          </Card>
          ...
          {/* ========== SECTION 10: Config ========== */}
          <Card id="config" title="Configuracion Avanzada" icon={<Settings size={16} className="text-[var(--accent)]" />}>
            <div className="space-y-5">
              {/* Related */}
              <div>
                ...
                <p className="text-[10px] text-[var(--text-muted)] mt-1">Aparecen en "Tambien te puede interesar" en la tienda.</p>
              </div>
              ...
              <div className="border-t border-[var(--border)] pt-4 opacity-60">
                <p className="text-[10px] text-[var(--text-muted)] mb-2">Producto SaaS flags (solo visible para Owner/Dev)</p>
                <div className="space-y-1.5">
                ...
                {isEditing && <p className="text-[10px] text-[var(--text-muted)] mt-2">Medusa ID: prod_01KJ4E6ZFNMMVE9SQES7TJDH1J</p>}
              </div>
            </div>
          </Card>
          ...
          {/* ========== SECTION 11: Stats ========== */}
          {isEditing && product && (
            <Card id="stats" title="Estadisticas" icon={<BarChart3 size={16} className="text-[var(--accent)]" />}>
              <div className="space-y-5">
                ...
                <p className="text-xs text-[var(--text-secondary)]">Tasa de conversion: <span className="text-[var(--text)]">3.4%</span> (visitas → compra)</p>
                ...
                <div className="h-40">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={salesChartData}>
                      ...
                      <Line type="monotone" dataKey="ventas" stroke="var(--accent)" strokeWidth={2} dot={{ r: 3, fill: 'var(--accent)' }} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
                ...
                <div className="flex items-center gap-2 text-xs text-[var(--text-secondary)]">
                  <span>Reviews:</span>
                  <span className="text-[var(--accent)]">★ {product.avg_rating}</span>
                ...
                {form.variants.length > 0 && (
                  <div>
                    <p className="text-xs text-[var(--text)] mb-2">Variantes mas vendidas:</p>
                    <div className="space-y-1.5">
                      {[
                ...
                {product.laser_available && (
                  <p className="text-xs text-[var(--text-secondary)]">Con grabado laser: <span className="text-[var(--text)]">65%</span> de las ventas</p>
                )}

                {/* Frequently bought together */}
                <div>
                  <p className="text-xs text-[var(--text)] mb-2">Frecuentemente comprado con:</p>
      ...
      {/* ===== STICKY FOOTER ===== */}
      {hasChanges && (
        <div className="fixed bottom-0 left-0 right-0 z-40 bg-[var(--surface)] border-t border-[var(--border)] shadow-lg px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-4 py-2 text-xs text-[var(--text-secondary)] hover:text-[var(--text)] transition-colors"
            >
              Descartar cambios
            </button>
            {isEditing ? (
