  ...
  if (confirming) {
    return (
      <span className="inline-flex items-center gap-1">
        ...
        <button onClick={() => setConfirming(false)} className="px-2 py-0.5 bg-wood-200 text-[var(--admin-text)] text-[10px] font-bold rounded hover:bg-wood-300 transition-colors">Cancelar</button>
      </span>
    );
  }
...
export const QuotePricingPanel: React.FC = () => {
  ...
  if (loading) {
    return <div className="flex items-center justify-center py-20"><Loader2 className="w-6 h-6 animate-spin text-[var(--admin-muted)]" /></div>;
  }
  ...
  return (
    <div className="space-y-6">
        ...
        {SECTIONS.map(s => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider whitespace-nowrap transition-all ${
              section === s.id
                ? 'bg-wood-900 dark:bg-[var(--admin-surface2)] text-sand-100 dark:text-[var(--admin-text)]'
                : 'text-[var(--admin-muted)] hover:bg-[var(--admin-surface2)] dark:hover:bg-wood-800'
            }`}
          >
            {s.label}
          </button>
      ...
      {/* Content */}
      <div className="bg-[var(--admin-surface)] dark:bg-wood-950 rounded-xl border border-[var(--admin-border)] dark:border-wood-800 p-6 min-h-[400px]">
      ...
      {/* Save bar */}
      {dirty && (
        <div className="sticky bottom-4 flex items-center justify-between bg-wood-900 dark:bg-[var(--admin-surface2)] text-sand-100 dark:text-[var(--admin-text)] rounded-xl px-6 py-4 shadow-xl">
          <span className="text-sm font-medium">Cambios sin guardar</span>
          <div className="flex gap-3">
            <button onClick={handleUndo} className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-[var(--admin-surface)]/10 hover:bg-[var(--admin-surface)]/20 text-xs font-bold uppercase">
              <RotateCcw className="w-3.5 h-3.5" /> Deshacer
            </button>
            <button onClick={handleSave} disabled={saving} className="flex items-center gap-1.5 px-6 py-2 rounded-lg bg-[var(--admin-accent)] text-[var(--admin-text)] text-xs font-bold uppercase hover:shadow-lg disabled:opacity-50">
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-8 right-8 bg-wood-900 dark:bg-[var(--admin-surface2)] text-sand-100 dark:text-[var(--admin-text)] px-6 py-3 rounded-xl shadow-xl text-sm font-medium z-50">
          {toast}
        </div>
      )}
    </div>
...
function EnableToggle({ enabled, onChange }: { enabled: boolean; onChange: (v: boolean) => void }) {
  return (
    <button onClick={() => onChange(!enabled)} className="shrink-0">
      {enabled ? <ToggleRight className="w-6 h-6 text-green-500" /> : <ToggleLeft className="w-6 h-6 text-[var(--admin-muted)]" />}
    </button>
  );
}
...
function SectionTitle({ title, subtitle }: { title: string; subtitle?: string }) {
  return (
    <div className="mb-6">
      <h3 className="text-lg font-serif font-bold text-[var(--admin-text)] dark:text-sand-100">{title}</h3>
  ...
  return (
    <div className="flex flex-col gap-0.5 shrink-0">
      <button disabled={index === 0} onClick={() => onMove(index, index - 1)}
        className="p-0.5 text-[var(--admin-muted)] hover:text-[var(--admin-text-secondary)] disabled:opacity-20 transition-colors">
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        {config.woodProducts.map((p, i) => (
          <div key={i} className="flex items-center gap-3 px-4 py-3 bg-[var(--admin-surface2)] dark:bg-wood-900/50 rounded-lg">
            ...
            <div className="flex-1 min-w-0">
              <input value={p.label} onChange={(e) => { const arr = [...config.woodProducts]; arr[i] = { ...p, label: e.target.value }; onChange({ woodProducts: arr }); }}
                className="font-medium text-sm text-[var(--admin-text)] dark:text-sand-100 bg-transparent border-b border-transparent hover:border-[var(--admin-border)] focus:border-[var(--admin-accent)] outline-none w-full" />
              <input value={p.desc} onChange={(e) => { const arr = [...config.woodProducts]; arr[i] = { ...p, desc: e.target.value }; onChange({ woodProducts: arr }); }}
                className="text-xs text-[var(--admin-muted)] bg-transparent border-b border-transparent hover:border-[var(--admin-border)] focus:border-[var(--admin-accent)] outline-none w-full mt-0.5" placeholder="Descripción..." />
            </div>
            <button onClick={() => removeProduct('wood', i)} className="p-1 text-[var(--admin-muted)] hover:text-red-500 transition-colors shrink-0"><Trash2 className="w-3.5 h-3.5" /></button>
          </div>
        ))}
      </div>
      {addingTo === 'wood' ? (
        <div className="flex items-center gap-2 px-4 py-2 bg-[var(--admin-accent)]/5 border border-[var(--admin-accent)]/20 rounded-lg">
          ...
          <input value={newProd.desc} onChange={e => setNewProd(p => ({ ...p, desc: e.target.value }))} placeholder="Descripción" className="flex-1 text-xs bg-transparent outline-none text-[var(--admin-muted)]" />
          <button onClick={() => handleAddProduct('wood')} className="text-xs text-[var(--admin-accent)] font-bold hover:underline">Agregar</button>
          <button onClick={() => { setAddingTo(null); setNewProd({ type: '', label: '', desc: '' }); }} className="text-xs text-[var(--admin-muted)]">✕</button>
        </div>
      ) : (
        <button onClick={() => setAddingTo('wood')} className="flex items-center gap-1.5 text-xs font-bold text-[var(--admin-accent)] hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar producto de madera</button>
      )}
      ...
      <div className="space-y-2">
        {config.textileProducts.map((p, i) => (
          <div key={i} className="flex items-center gap-3 px-4 py-3 bg-[var(--admin-surface2)] dark:bg-wood-900/50 rounded-lg">
            ...
            <button onClick={() => removeProduct('textile', i)} className="p-1 text-[var(--admin-muted)] hover:text-red-500 transition-colors shrink-0"><Trash2 className="w-3.5 h-3.5" /></button>
          </div>
        ))}
      </div>
      {addingTo === 'textile' ? (
        <div className="flex items-center gap-2 px-4 py-2 bg-[var(--admin-accent)]/5 border border-[var(--admin-accent)]/20 rounded-lg">
          <input value={newProd.label} onChange={e => setNewProd(p => ({ ...p, label: e.target.value, type: e.target.value }))} placeholder="Nombre del producto" className="flex-1 text-sm bg-transparent outline-none" autoFocus />
          <input value={newProd.desc} onChange={e => setNewProd(p => ({ ...p, desc: e.target.value }))} placeholder="Descripción" className="flex-1 text-xs bg-transparent outline-none text-[var(--admin-muted)]" />
          <button onClick={() => handleAddProduct('textile')} className="text-xs text-[var(--admin-accent)] font-bold hover:underline">Agregar</button>
          <button onClick={() => { setAddingTo(null); setNewProd({ type: '', label: '', desc: '' }); }} className="text-xs text-[var(--admin-muted)]">✕</button>
        </div>
      ) : (
        <button onClick={() => setAddingTo('textile')} className="flex items-center gap-1.5 text-xs font-bold text-[var(--admin-accent)] hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar producto textil</button>
      )}

      <SectionTitle title="Servicio de Grabado" subtitle="Categoría de grabado láser como servicio" />
      <div className="flex items-center gap-4 px-4 py-3 bg-[var(--admin-surface2)] dark:bg-wood-900/50 rounded-lg">
        ...
        <input value={config.serviceProduct.label} onChange={(e) => onChange({ serviceProduct: { ...config.serviceProduct, label: e.target.value } })}
          className="font-medium text-sm bg-transparent border-b border-transparent hover:border-[var(--admin-border)] focus:border-[var(--admin-accent)] outline-none" />
      </div>
    </div>
  );
}
          ...
          <div key={i} className="flex items-center gap-3 px-4 py-3 bg-[var(--admin-surface2)] dark:bg-wood-900/50 rounded-lg">
            ...
            <div className="flex-1 min-w-0">
              <input value={w.label} onChange={(e) => { const arr = [...config.woodOptions]; arr[i] = { ...w, label: e.target.value }; onChange({ woodOptions: arr }); }}
                className="font-medium text-sm bg-transparent border-b border-transparent hover:border-[var(--admin-border)] focus:border-[var(--admin-accent)] outline-none w-full" />
              <input value={w.description} onChange={(e) => { const arr = [...config.woodOptions]; arr[i] = { ...w, description: e.target.value }; onChange({ woodOptions: arr }); }}
                className="text-xs text-[var(--admin-muted)] bg-transparent border-b border-transparent hover:border-[var(--admin-border)] focus:border-[var(--admin-accent)] outline-none w-full mt-0.5" />
            </div>
            <div className="flex items-center gap-1.5 shrink-0">
              <span className="text-[10px] text-[var(--admin-muted)]">$/m²</span>
              ...
              }} className="w-20 px-2 py-1 text-right text-sm bg-[var(--admin-surface)] dark:bg-wood-800 border border-[var(--admin-border)] dark:border-wood-700 rounded focus:border-[var(--admin-accent)] outline-none" />
            </div>
            <button onClick={() => { onChange({ woodOptions: config.woodOptions.filter((_, j) => j !== i) }); }}
              className="p-1 text-[var(--admin-muted)] hover:text-red-500 transition-colors shrink-0"><Trash2 className="w-3.5 h-3.5" /></button>
          </div>
        ))}
      </div>
      {adding ? (
        <div className="flex items-center gap-2 px-4 py-3 bg-[var(--admin-accent)]/5 border border-[var(--admin-accent)]/20 rounded-lg">
          ...
          <input value={newWood.description} onChange={e => setNewWood(p => ({ ...p, description: e.target.value }))} placeholder="Descripción" className="flex-1 text-xs bg-transparent outline-none text-[var(--admin-muted)]" />
          <input type="number" value={newWood.priceM2} onChange={e => setNewWood(p => ({ ...p, priceM2: Number(e.target.value) }))} className="w-20 px-2 py-1 text-right text-sm border border-[var(--admin-border)] rounded outline-none" />
          <button onClick={() => { if (!newWood.label.trim()) return; onChange({ woodOptions: [...config.woodOptions, { ...newWood, gradient: `linear-gradient(135deg, ${newWood.color} 0%, ${newWood.color}88 100%)`, enabled: true }] }); setNewWood({ label: '', color: '#8B6914', description: '', priceM2: 4000 }); setAdding(false); }} className="text-xs text-[var(--admin-accent)] font-bold hover:underline">Agregar</button>
          <button onClick={() => setAdding(false)} className="text-xs text-[var(--admin-muted)]">✕</button>
        </div>
      ) : (
        <button onClick={() => setAdding(true)} className="flex items-center gap-1.5 text-xs font-bold text-[var(--admin-accent)] hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar tipo de madera</button>
      )}
      <div className="flex gap-4">
        <div className="flex-1">
          <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase tracking-widest block mb-1">Precio mínimo pieza</label>
          <input type="number" value={config.woodMinPrice} onChange={(e) => onChange({ woodMinPrice: Number(e.target.value) })}
            className="w-full px-3 py-2 text-sm bg-[var(--admin-surface)] dark:bg-wood-800 border border-[var(--admin-border)] dark:border-wood-700 rounded-lg focus:border-[var(--admin-accent)] outline-none" />
        </div>
        <div className="flex-1">
          <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase tracking-widest block mb-1">Espesor estándar (cm)</label>
    ...
    <div className="space-y-6">
        ...
        {config.textileColors.map((c, i) => (
          <div key={i} className="flex items-center gap-3 px-4 py-3 bg-[var(--admin-surface2)] dark:bg-wood-900/50 rounded-lg">
            ...
            <input value={c.label} onChange={(e) => { const arr = [...config.textileColors]; arr[i] = { ...c, label: e.target.value }; onChange({ textileColors: arr }); }}
              className="flex-1 font-medium text-sm bg-transparent border-b border-transparent hover:border-[var(--admin-border)] focus:border-[var(--admin-accent)] outline-none" />
            <button onClick={() => { onChange({ textileColors: config.textileColors.filter((_, j) => j !== i) }); }}
              className="p-1 text-[var(--admin-muted)] hover:text-red-500 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
          </div>
        ))}
      </div>
      {addingColor ? (
        <div className="flex items-center gap-2 px-4 py-2 bg-[var(--admin-accent)]/5 border border-[var(--admin-accent)]/20 rounded-lg">
          ...
          <button onClick={() => { if (!newColor.label.trim()) return; onChange({ textileColors: [...config.textileColors, { ...newColor, enabled: true }] }); setNewColor({ label: '', hex: '#CCCCCC' }); setAddingColor(false); }} className="text-xs text-[var(--admin-accent)] font-bold hover:underline">Agregar</button>
          <button onClick={() => setAddingColor(false)} className="text-xs text-[var(--admin-muted)]">✕</button>
        </div>
      ) : (
        <button onClick={() => setAddingColor(true)} className="flex items-center gap-1.5 text-xs font-bold text-[var(--admin-accent)] hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar color</button>
      )}
      ...
      {addingTech ? (
        <div className="flex items-center gap-2 px-4 py-2 bg-[var(--admin-accent)]/5 border border-[var(--admin-accent)]/20 rounded-lg">
          ...
          <input type="number" value={newTech.price} onChange={e => setNewTech(p => ({ ...p, price: Number(e.target.value) }))} className="w-20 px-2 py-1 text-right text-sm border border-[var(--admin-border)] rounded outline-none" />
          <button onClick={() => { if (!newTech.label.trim()) return; onChange({ textileTechniques: [...config.textileTechniques, { ...newTech, enabled: true }] }); setNewTech({ label: '', price: 50 }); setAddingTech(false); }} className="text-xs text-[var(--admin-accent)] font-bold hover:underline">Agregar</button>
          <button onClick={() => setAddingTech(false)} className="text-xs text-[var(--admin-muted)]">✕</button>
        </div>
      ) : (
        <button onClick={() => setAddingTech(true)} className="flex items-center gap-1.5 text-xs font-bold text-[var(--admin-accent)] hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar técnica</button>
      )}
      <div>
        <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase tracking-widest block mb-1">Extra panel completo ($)</label>
        <input type="number" value={config.textileFullPanelExtra} onChange={(e) => onChange({ textileFullPanelExtra: Number(e.target.value) })}
          className="w-32 px-3 py-2 text-sm bg-[var(--admin-surface)] dark:bg-wood-800 border border-[var(--admin-border)] dark:border-wood-700 rounded-lg focus:border-[var(--admin-accent)] outline-none" />
      </div>
    </div>
  );
}
    ...
    <div className="space-y-6">
      ...
      {addingLevel ? (
        <div className="flex items-center gap-2 px-4 py-2 bg-[var(--admin-accent)]/5 border border-[var(--admin-accent)]/20 rounded-lg">
          ...
          <input type="number" value={newLevel.price} onChange={e => setNewLevel(p => ({ ...p, price: Number(e.target.value) }))} className="w-20 px-2 py-1 text-right text-sm border border-[var(--admin-border)] rounded outline-none" />
          <button onClick={() => { if (!newLevel.complexity.trim()) return; onChange({ engravingPrices: [...config.engravingPrices, newLevel] }); setNewLevel({ complexity: '', price: 100 }); setAddingLevel(false); }} className="text-xs text-[var(--admin-accent)] font-bold hover:underline">Agregar</button>
          <button onClick={() => setAddingLevel(false)} className="text-xs text-[var(--admin-muted)]">✕</button>
        </div>
      ) : (
        <button onClick={() => setAddingLevel(true)} className="flex items-center gap-1.5 text-xs font-bold text-[var(--admin-accent)] hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar nivel de complejidad</button>
      )}

      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase tracking-widest block mb-1">Extra por zona adicional ($)</label>
          <input type="number" value={config.engravingZoneExtra} onChange={(e) => onChange({ engravingZoneExtra: Number(e.target.value) })}
            className="w-full px-3 py-2 text-sm bg-[var(--admin-surface)] dark:bg-wood-800 border border-[var(--admin-border)] dark:border-wood-700 rounded-lg focus:border-[var(--admin-accent)] outline-none" />
        </div>
        <div>
          <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase tracking-widest block mb-1">Extra código QR ($)</label>
      ...
      <div className="space-y-2">
        {config.engraveMaterials.map((m, i) => (
          <div key={i} className="flex items-center gap-3 px-4 py-2 bg-[var(--admin-surface2)] dark:bg-wood-900/50 rounded-lg">
            ...
            <input value={m.label} onChange={(e) => { const arr = [...config.engraveMaterials]; arr[i] = { ...m, label: e.target.value }; onChange({ engraveMaterials: arr }); }}
              className="flex-1 text-sm bg-transparent border-b border-transparent hover:border-[var(--admin-border)] focus:border-[var(--admin-accent)] outline-none" />
      ...
      {addingMat ? (
        <div className="flex items-center gap-2 px-4 py-2 bg-[var(--admin-accent)]/5 border border-[var(--admin-accent)]/20 rounded-lg">
          ...
          <button onClick={() => { if (!newMat.trim()) return; onChange({ engraveMaterials: [...config.engraveMaterials, { label: newMat, enabled: true }] }); setNewMat(''); setAddingMat(false); }} className="text-xs text-[var(--admin-accent)] font-bold hover:underline">Agregar</button>
          <button onClick={() => setAddingMat(false)} className="text-xs text-[var(--admin-muted)]">✕</button>
        </div>
      ) : (
        <button onClick={() => setAddingMat(true)} className="flex items-center gap-1.5 text-xs font-bold text-[var(--admin-accent)] hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar material</button>
      )}
    </div>
  );
}
    ...
    <div className="space-y-6">
      ...
      {addingShape ? (
        <div className="flex items-center gap-2 px-4 py-2 bg-[var(--admin-accent)]/5 border border-[var(--admin-accent)]/20 rounded-lg">
          ...
          <button onClick={() => { if (!newShape.trim()) return; onChange({ boardShapes: [...config.boardShapes, { value: newShape, label: newShape, enabled: true }] }); setNewShape(''); setAddingShape(false); }} className="text-xs text-[var(--admin-accent)] font-bold hover:underline">Agregar</button>
          <button onClick={() => setAddingShape(false)} className="text-xs text-[var(--admin-muted)]">✕</button>
        </div>
      ) : (
        <button onClick={() => setAddingShape(true)} className="flex items-center gap-1.5 text-xs font-bold text-[var(--admin-accent)] hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar forma</button>
      )}
      ...
      {addingExtra ? (
        <div className="flex items-center gap-2 px-4 py-2 bg-[var(--admin-accent)]/5 border border-[var(--admin-accent)]/20 rounded-lg">
          ...
          <input value={newExtra.desc} onChange={e => setNewExtra(p => ({ ...p, desc: e.target.value }))} placeholder="Descripción" className="flex-1 text-xs bg-transparent outline-none text-[var(--admin-muted)]" />
          <input type="number" value={newExtra.priceExtra} onChange={e => setNewExtra(p => ({ ...p, priceExtra: Number(e.target.value) }))} className="w-16 px-2 py-1 text-right text-sm border border-[var(--admin-border)] rounded outline-none" placeholder="$" />
          <button onClick={() => { if (!newExtra.label.trim()) return; onChange({ boardExtras: [...config.boardExtras, { key: newExtra.label.toLowerCase().replace(/\s+/g, '_'), ...newExtra, enabled: true }] }); setNewExtra({ label: '', desc: '', priceExtra: 0 }); setAddingExtra(false); }} className="text-xs text-[var(--admin-accent)] font-bold hover:underline">Agregar</button>
          <button onClick={() => setAddingExtra(false)} className="text-xs text-[var(--admin-muted)]">✕</button>
        </div>
      ) : (
        <button onClick={() => setAddingExtra(true)} className="flex items-center gap-1.5 text-xs font-bold text-[var(--admin-accent)] hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar extra</button>
      )}
      ...
      {addingFinish ? (
        <div className="flex items-center gap-2 px-4 py-2 bg-[var(--admin-accent)]/5 border border-[var(--admin-accent)]/20 rounded-lg">
          ...
          <button onClick={() => { if (!newFinish.trim()) return; onChange({ boardFinishes: [...config.boardFinishes, { label: newFinish, enabled: true }] }); setNewFinish(''); setAddingFinish(false); }} className="text-xs text-[var(--admin-accent)] font-bold hover:underline">Agregar</button>
          <button onClick={() => setAddingFinish(false)} className="text-xs text-[var(--admin-muted)]">✕</button>
        </div>
      ) : (
        <button onClick={() => setAddingFinish(true)} className="flex items-center gap-1.5 text-xs font-bold text-[var(--admin-accent)] hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar acabado</button>
      )}
    </div>
  );
}
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        {config.volumeDiscounts.map((d, i) => (
          <div key={i} className="flex items-center gap-4 px-4 py-3 bg-[var(--admin-surface2)] dark:bg-wood-900/50 rounded-lg">
            <span className="text-xs text-[var(--admin-muted)] shrink-0">A partir de</span>
            <input type="number" value={d.min_qty}
              ...
              className="w-16 px-2 py-1 text-center text-sm bg-[var(--admin-surface)] dark:bg-wood-800 border border-[var(--admin-border)] dark:border-wood-700 rounded focus:border-[var(--admin-accent)] outline-none" />
            <span className="text-xs text-[var(--admin-muted)]">piezas →</span>
            ...
            <button onClick={() => removeDiscount(i)} className="p-1 text-[var(--admin-muted)] hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
          </div>
        ))}
      </div>
      <button onClick={addDiscount} className="flex items-center gap-2 text-xs font-bold text-[var(--admin-accent)] hover:underline">
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {config.bundles.map((b, i) => {
          ...
          return (
            <div key={b.id} className="bg-[var(--admin-surface2)] dark:bg-wood-900/50 rounded-xl border border-[var(--admin-border)] dark:border-wood-800 overflow-hidden">
              {/* Header */}
              <div className="flex items-center gap-3 px-4 py-3">
                ...
                <button onClick={() => setExpandedId(isExpanded ? null : b.id)} className="flex-1 flex items-center gap-3 text-left min-w-0">
                  <div className="flex-1 min-w-0">
                    <span className="font-bold text-sm text-[var(--admin-text)] dark:text-sand-100 block truncate">{b.name}</span>
                    <span className="text-[10px] text-[var(--admin-muted)]">{b.segment || 'Sin segmento'} · {b.items.length} items</span>
                  </div>
                  {isExpanded ? <ChevronUp className="w-4 h-4 text-[var(--admin-muted)] shrink-0" /> : <ChevronDown className="w-4 h-4 text-[var(--admin-muted)] shrink-0" />}
                </button>
                <div className="flex items-center gap-1.5 shrink-0">
                  <input type="number" value={b.discountPercent}
                    onChange={(e) => updateBundle(i, { discountPercent: Number(e.target.value) })}
                    className="w-14 px-2 py-1 text-center text-sm bg-[var(--admin-surface)] dark:bg-wood-800 border border-[var(--admin-border)] dark:border-wood-700 rounded focus:border-[var(--admin-accent)] outline-none" />
                  <span className="text-[10px] text-[var(--admin-muted)]">%</span>
                </div>
                <button onClick={() => handleDelete(i)} className="p-1.5 text-[var(--admin-muted)] hover:text-red-500 shrink-0 transition-colors"><Trash2 className="w-4 h-4" /></button>
              </div>

              {/* Expanded content */}
              {isExpanded && (
                <div className="px-4 pb-4 space-y-4 border-t border-[var(--admin-border)] dark:border-wood-800 pt-3">
                  {/* Editable fields */}
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                      <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase block mb-1">Nombre</label>
                      <input value={b.name} onChange={(e) => updateBundle(i, { name: e.target.value })}
                        className="w-full px-3 py-2 text-sm bg-[var(--admin-surface)] dark:bg-wood-800 border border-[var(--admin-border)] dark:border-wood-700 rounded-lg focus:border-[var(--admin-accent)] outline-none" />
                    </div>
                    <div>
                      <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase block mb-1">Segmento</label>
                      <input value={b.segment} onChange={(e) => updateBundle(i, { segment: e.target.value })}
                        placeholder="B2B, Novios, Restaurantes..."
                        className="w-full px-3 py-2 text-sm bg-[var(--admin-surface)] dark:bg-wood-800 border border-[var(--admin-border)] dark:border-wood-700 rounded-lg focus:border-[var(--admin-accent)] outline-none" />
                    </div>
                    <div>
                      <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase block mb-1">Descuento %</label>
                      <input type="number" value={b.discountPercent} onChange={(e) => updateBundle(i, { discountPercent: Number(e.target.value) })}
                        className="w-full px-3 py-2 text-sm bg-[var(--admin-surface)] dark:bg-wood-800 border border-[var(--admin-border)] dark:border-wood-700 rounded-lg focus:border-[var(--admin-accent)] outline-none" />
                    </div>
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase block mb-1">Descripción</label>
                  ...
                  {/* Items */}
                  <div>
                    <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase block mb-2">Productos del paquete</label>
                    <div className="space-y-2">
                      {b.items.map((item, j) => (
                        <div key={j} className="flex items-center gap-2 bg-[var(--admin-surface)] dark:bg-wood-800 rounded-lg p-2 border border-[var(--admin-border)] dark:border-wood-700">
                          ...
                          }} className="text-xs bg-[var(--admin-surface2)] dark:bg-wood-900 border border-[var(--admin-border)] dark:border-wood-600 rounded px-2 py-1.5 outline-none">
                            {categoryOptions.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
                          </select>
                          <select value={item.type} onChange={(e) => updateItem(i, j, { type: e.target.value })}
                            className="flex-1 text-xs bg-[var(--admin-surface2)] dark:bg-wood-900 border border-[var(--admin-border)] dark:border-wood-600 rounded px-2 py-1.5 outline-none min-w-0">
                          ...
                          <input type="number" min={1} value={item.quantity} onChange={(e) => updateItem(i, j, { quantity: Number(e.target.value) || 1 })}
                            className="w-14 text-center text-xs bg-[var(--admin-surface2)] dark:bg-wood-900 border border-[var(--admin-border)] dark:border-wood-600 rounded px-1 py-1.5 outline-none" />
                          <button onClick={() => removeItem(i, j)} className="p-1 text-[var(--admin-muted)] hover:text-red-500 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                        </div>
                      ))}
                    </div>
                    <button onClick={() => addItem(i)} className="flex items-center gap-1.5 text-xs font-bold text-[var(--admin-accent)] hover:underline mt-2">
      ...
      {/* Add new bundle */}
      {showAddForm ? (
        <div className="bg-[var(--admin-surface)] dark:bg-wood-900 rounded-xl border border-[var(--admin-accent)]/30 p-4 space-y-3">
          <h4 className="text-sm font-medium text-[var(--admin-text)] dark:text-sand-100">Nuevo Paquete</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase block mb-1">Nombre *</label>
              <input value={newBundle.name} onChange={e => setNewBundle(p => ({ ...p, name: e.target.value }))}
                placeholder="Kit Corporativo"
                className="w-full px-3 py-2 text-sm bg-[var(--admin-surface2)] dark:bg-wood-800 border border-[var(--admin-border)] dark:border-wood-700 rounded-lg focus:border-[var(--admin-accent)] outline-none" />
            </div>
            <div>
              <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase block mb-1">Segmento</label>
          ...
          <div className="flex items-center gap-3">
            <div>
              ...
              <input type="number" value={newBundle.discountPercent} onChange={e => setNewBundle(p => ({ ...p, discountPercent: Number(e.target.value) }))}
                className="w-20 px-3 py-2 text-sm bg-[var(--admin-surface2)] dark:bg-wood-800 border border-[var(--admin-border)] dark:border-wood-700 rounded-lg focus:border-[var(--admin-accent)] outline-none" />
            </div>
            <div className="flex items-center gap-2 mt-5">
              <button onClick={() => setShowAddForm(false)} className="px-4 py-2 text-xs text-[var(--admin-text-secondary)] hover:text-[var(--admin-text)]">Cancelar</button>
        ...
        <button onClick={() => setShowAddForm(true)} className="flex items-center gap-2 text-xs font-bold text-[var(--admin-accent)] hover:underline">
...
function GaleriaSection({ config, onChange }: { config: FullQuoteConfig; onChange: (p: Partial<FullQuoteConfig>) => void }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        {categories.map((c, i) => (
          <div key={c.id} className="flex items-center gap-3 px-4 py-2 bg-[var(--admin-surface2)] dark:bg-wood-900/50 rounded-lg">
      ...
      {addingCat ? (
        <div className="flex items-center gap-2 px-4 py-2 bg-[var(--admin-accent)]/5 border border-[var(--admin-accent)]/20 rounded-lg">
          ...
          <button onClick={handleAddCategory} className="text-xs text-[var(--admin-accent)] font-bold hover:underline">Agregar</button>
          <button onClick={() => setAddingCat(false)} className="text-xs text-[var(--admin-muted)]">✕</button>
        </div>
      ) : (
        <button onClick={() => setAddingCat(true)} className="flex items-center gap-1.5 text-xs font-bold text-[var(--admin-accent)] hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar categoría</button>
      )}
      ...
      <div className="space-y-3">
        {templates.map((t, i) => {
          ...
          return (
            <div key={t.id} className="bg-[var(--admin-surface2)] dark:bg-wood-900/50 rounded-xl border border-[var(--admin-border)] dark:border-wood-800 overflow-hidden">
              <div className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="w-10 h-10 shrink-0 text-[var(--admin-text-secondary)] rounded-lg bg-[var(--admin-surface)] dark:bg-wood-800 border border-[var(--admin-border)] dark:border-wood-700 overflow-hidden p-1">
                  {t.svgCode ? (
                    <div dangerouslySetInnerHTML={{ __html: t.svgCode }} className="w-full h-full [&>svg]:w-full [&>svg]:h-full" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-[8px] text-[var(--admin-muted)]">SVG</div>
                  )}
                </div>
                <button onClick={() => setExpandedId(isExpanded ? null : t.id)} className="flex-1 text-left min-w-0">
                  <span className="font-medium text-sm text-[var(--admin-text)] dark:text-sand-100 block truncate">{t.name}</span>
                  <span className="text-[10px] text-[var(--admin-muted)]">{cat?.label || t.category} · {t.applicableTo.join(', ')}</span>
                </button>
                {isExpanded ? <ChevronUp className="w-4 h-4 text-[var(--admin-muted)] shrink-0" /> : <ChevronDown className="w-4 h-4 text-[var(--admin-muted)] shrink-0" />}
              ...
              {isExpanded && (
                <div className="px-4 pb-4 space-y-3 border-t border-[var(--admin-border)] dark:border-wood-800 pt-3">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase block mb-1">Nombre</label>
                    ...
                    <div>
                      <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase block mb-1">Categoría</label>
                      <select value={t.category} onChange={e => updateTemplate(i, { category: e.target.value })}
                        className="w-full px-3 py-2 text-sm bg-[var(--admin-surface)] dark:bg-wood-800 border border-[var(--admin-border)] dark:border-wood-700 rounded-lg outline-none">
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase block mb-1">Código SVG</label>
                    <textarea value={t.svgCode} onChange={e => updateTemplate(i, { svgCode: e.target.value })}
                      rows={3} className="w-full px-3 py-2 text-xs font-mono bg-[var(--admin-surface)] dark:bg-wood-800 border border-[var(--admin-border)] dark:border-wood-700 rounded-lg focus:border-[var(--admin-accent)] outline-none resize-none" />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase block mb-1">Texto default</label>
                      <input value={t.defaultText || ''} onChange={e => updateTemplate(i, { defaultText: e.target.value })}
                        className="w-full px-3 py-2 text-sm bg-[var(--admin-surface)] dark:bg-wood-800 border border-[var(--admin-border)] dark:border-wood-700 rounded-lg focus:border-[var(--admin-accent)] outline-none" />
                    </div>
                    <div>
                      <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase block mb-1">Tags (coma separados)</label>
                      <input value={t.tags.join(', ')} onChange={e => updateTemplate(i, { tags: e.target.value.split(',').map(s => s.trim()).filter(Boolean) })}
                        className="w-full px-3 py-2 text-sm bg-[var(--admin-surface)] dark:bg-wood-800 border border-[var(--admin-border)] dark:border-wood-700 rounded-lg focus:border-[var(--admin-accent)] outline-none" />
                    </div>
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase block mb-1">Aplica a</label>
                    <div className="flex gap-2">
                      {(['madera', 'textil', 'grabado'] as const).map(cat => {
                        ...
                        return (
                          ...
                          }} className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${active ? 'border-[var(--admin-accent)] bg-[var(--admin-accent)]/10 text-[var(--admin-text)]' : 'border-[var(--admin-border)] text-[var(--admin-muted)]'}`}>
      ...
      {addingTemplate ? (
        <div className="bg-[var(--admin-surface)] dark:bg-wood-900 rounded-xl border border-[var(--admin-accent)]/30 p-4 space-y-3">
          <h4 className="text-sm font-medium text-[var(--admin-text)] dark:text-sand-100">Nuevo Template</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-bold text-[var(--admin-muted)] uppercase block mb-1">Nombre *</label>
            ...
            <div>
              ...
              <select value={newTpl.category} onChange={e => setNewTpl(p => ({ ...p, category: e.target.value }))}
                className="w-full px-3 py-2 text-sm bg-[var(--admin-surface2)] dark:bg-wood-800 border border-[var(--admin-border)] dark:border-wood-700 rounded-lg outline-none">
                {categories.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}
              </select>
            </div>
          </div>
          ...
          <div>
            ...
            <textarea value={newTpl.svgCode} onChange={e => setNewTpl(p => ({ ...p, svgCode: e.target.value }))}
              ...
              className="w-full px-3 py-2 text-xs font-mono bg-[var(--admin-surface2)] dark:bg-wood-800 border border-[var(--admin-border)] dark:border-wood-700 rounded-lg focus:border-[var(--admin-accent)] outline-none resize-none" />
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => setAddingTemplate(false)} className="px-4 py-2 text-xs text-[var(--admin-text-secondary)] hover:text-[var(--admin-text)]">Cancelar</button>
        ...
        <button onClick={() => setAddingTemplate(true)} className="flex items-center gap-1.5 text-xs font-bold text-[var(--admin-accent)] hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar template de diseño</button>
      )}
    </div>
  );
}
