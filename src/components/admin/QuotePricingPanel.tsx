  ...
  return (
    <div className="flex flex-col gap-0.5 shrink-0">
      <button disabled={index === 0} onClick={() => onMove(index, index - 1)}
        className="p-0.5 text-wood-300 hover:text-wood-600 disabled:opacity-20 transition-colors">
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        {config.woodProducts.map((p, i) => (
          <div key={i} className="flex items-center gap-3 px-4 py-3 bg-sand-50 dark:bg-wood-900/50 rounded-lg">
            ...
            <div className="flex-1 min-w-0">
              <input value={p.label} onChange={(e) => { const arr = [...config.woodProducts]; arr[i] = { ...p, label: e.target.value }; onChange({ woodProducts: arr }); }}
                className="font-medium text-sm text-wood-900 dark:text-sand-100 bg-transparent border-b border-transparent hover:border-wood-200 focus:border-accent-gold outline-none w-full" />
              <input value={p.desc} onChange={(e) => { const arr = [...config.woodProducts]; arr[i] = { ...p, desc: e.target.value }; onChange({ woodProducts: arr }); }}
                className="text-xs text-wood-400 bg-transparent border-b border-transparent hover:border-wood-200 focus:border-accent-gold outline-none w-full mt-0.5" placeholder="Descripción..." />
            </div>
            <button onClick={() => removeProduct('wood', i)} className="p-1 text-wood-300 hover:text-red-500 transition-colors shrink-0"><Trash2 className="w-3.5 h-3.5" /></button>
          </div>
        ))}
      </div>
      {addingTo === 'wood' ? (
        <div className="flex items-center gap-2 px-4 py-2 bg-accent-gold/5 border border-accent-gold/20 rounded-lg">
          ...
          <input value={newProd.desc} onChange={e => setNewProd(p => ({ ...p, desc: e.target.value }))} placeholder="Descripción" className="flex-1 text-xs bg-transparent outline-none text-wood-400" />
          <button onClick={() => handleAddProduct('wood')} className="text-xs text-accent-gold font-bold hover:underline">Agregar</button>
          <button onClick={() => { setAddingTo(null); setNewProd({ type: '', label: '', desc: '' }); }} className="text-xs text-wood-400">✕</button>
        </div>
      ) : (
        <button onClick={() => setAddingTo('wood')} className="flex items-center gap-1.5 text-xs font-bold text-accent-gold hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar producto de madera</button>
      )}
      ...
      <div className="space-y-2">
        {config.textileProducts.map((p, i) => (
          <div key={i} className="flex items-center gap-3 px-4 py-3 bg-sand-50 dark:bg-wood-900/50 rounded-lg">
            ...
            <button onClick={() => removeProduct('textile', i)} className="p-1 text-wood-300 hover:text-red-500 transition-colors shrink-0"><Trash2 className="w-3.5 h-3.5" /></button>
          </div>
        ))}
      </div>
      {addingTo === 'textile' ? (
        <div className="flex items-center gap-2 px-4 py-2 bg-accent-gold/5 border border-accent-gold/20 rounded-lg">
          <input value={newProd.label} onChange={e => setNewProd(p => ({ ...p, label: e.target.value, type: e.target.value }))} placeholder="Nombre del producto" className="flex-1 text-sm bg-transparent outline-none" autoFocus />
          <input value={newProd.desc} onChange={e => setNewProd(p => ({ ...p, desc: e.target.value }))} placeholder="Descripción" className="flex-1 text-xs bg-transparent outline-none text-wood-400" />
          <button onClick={() => handleAddProduct('textile')} className="text-xs text-accent-gold font-bold hover:underline">Agregar</button>
          <button onClick={() => { setAddingTo(null); setNewProd({ type: '', label: '', desc: '' }); }} className="text-xs text-wood-400">✕</button>
        </div>
      ) : (
        <button onClick={() => setAddingTo('textile')} className="flex items-center gap-1.5 text-xs font-bold text-accent-gold hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar producto textil</button>
      )}
      ...
      <div className="flex items-center gap-4 px-4 py-3 bg-sand-50 dark:bg-wood-900/50 rounded-lg">
        ...
        <input value={config.serviceProduct.label} onChange={(e) => onChange({ serviceProduct: { ...config.serviceProduct, label: e.target.value } })}
          className="font-medium text-sm bg-transparent border-b border-transparent hover:border-wood-200 focus:border-accent-gold outline-none" />
      </div>
    </div>
  );
}
...
function MaderasSection({ config, onChange }: { config: FullQuoteConfig; onChange: (p: Partial<FullQuoteConfig>) => void }) {
  ...
  return (
    <div className="space-y-6">
          ...
          <div key={i} className="flex items-center gap-3 px-4 py-3 bg-sand-50 dark:bg-wood-900/50 rounded-lg">
            ...
            <div className="flex-1 min-w-0">
              <input value={w.label} onChange={(e) => { const arr = [...config.woodOptions]; arr[i] = { ...w, label: e.target.value }; onChange({ woodOptions: arr }); }}
                className="font-medium text-sm bg-transparent border-b border-transparent hover:border-wood-200 focus:border-accent-gold outline-none w-full" />
              <input value={w.description} onChange={(e) => { const arr = [...config.woodOptions]; arr[i] = { ...w, description: e.target.value }; onChange({ woodOptions: arr }); }}
                className="text-xs text-wood-400 bg-transparent border-b border-transparent hover:border-wood-200 focus:border-accent-gold outline-none w-full mt-0.5" />
            </div>
      ...
      {adding ? (
        <div className="flex items-center gap-2 px-4 py-3 bg-accent-gold/5 border border-accent-gold/20 rounded-lg">
          ...
          <input value={newWood.description} onChange={e => setNewWood(p => ({ ...p, description: e.target.value }))} placeholder="Descripción" className="flex-1 text-xs bg-transparent outline-none text-wood-400" />
          <input type="number" value={newWood.priceM2} onChange={e => setNewWood(p => ({ ...p, priceM2: Number(e.target.value) }))} className="w-20 px-2 py-1 text-right text-sm border border-wood-200 rounded outline-none" />
          <button onClick={() => { if (!newWood.label.trim()) return; onChange({ woodOptions: [...config.woodOptions, { ...newWood, gradient: `linear-gradient(135deg, ${newWood.color} 0%, ${newWood.color}88 100%)`, enabled: true }] }); setNewWood({ label: '', color: '#8B6914', description: '', priceM2: 4000 }); setAdding(false); }} className="text-xs text-accent-gold font-bold hover:underline">Agregar</button>
          <button onClick={() => setAdding(false)} className="text-xs text-wood-400">✕</button>
        </div>
      ) : (
        <button onClick={() => setAdding(true)} className="flex items-center gap-1.5 text-xs font-bold text-accent-gold hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar tipo de madera</button>
      )}
    ...
    <div className="space-y-6">
      ...
      {addingColor ? (
        <div className="flex items-center gap-2 px-4 py-2 bg-accent-gold/5 border border-accent-gold/20 rounded-lg">
          ...
          <button onClick={() => { if (!newColor.label.trim()) return; onChange({ textileColors: [...config.textileColors, { ...newColor, enabled: true }] }); setNewColor({ label: '', hex: '#CCCCCC' }); setAddingColor(false); }} className="text-xs text-accent-gold font-bold hover:underline">Agregar</button>
          <button onClick={() => setAddingColor(false)} className="text-xs text-wood-400">✕</button>
        </div>
      ) : (
        <button onClick={() => setAddingColor(true)} className="flex items-center gap-1.5 text-xs font-bold text-accent-gold hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar color</button>
      )}
      ...
      {addingTech ? (
        <div className="flex items-center gap-2 px-4 py-2 bg-accent-gold/5 border border-accent-gold/20 rounded-lg">
          ...
          <input type="number" value={newTech.price} onChange={e => setNewTech(p => ({ ...p, price: Number(e.target.value) }))} className="w-20 px-2 py-1 text-right text-sm border border-wood-200 rounded outline-none" />
          <button onClick={() => { if (!newTech.label.trim()) return; onChange({ textileTechniques: [...config.textileTechniques, { ...newTech, enabled: true }] }); setNewTech({ label: '', price: 50 }); setAddingTech(false); }} className="text-xs text-accent-gold font-bold hover:underline">Agregar</button>
          <button onClick={() => setAddingTech(false)} className="text-xs text-wood-400">✕</button>
        </div>
      ) : (
        <button onClick={() => setAddingTech(true)} className="flex items-center gap-1.5 text-xs font-bold text-accent-gold hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar técnica</button>
      )}
      <div>
    ...
    <div className="space-y-6">
      ...
      {addingLevel ? (
        <div className="flex items-center gap-2 px-4 py-2 bg-accent-gold/5 border border-accent-gold/20 rounded-lg">
          ...
          <input type="number" value={newLevel.price} onChange={e => setNewLevel(p => ({ ...p, price: Number(e.target.value) }))} className="w-20 px-2 py-1 text-right text-sm border border-wood-200 rounded outline-none" />
          <button onClick={() => { if (!newLevel.complexity.trim()) return; onChange({ engravingPrices: [...config.engravingPrices, newLevel] }); setNewLevel({ complexity: '', price: 100 }); setAddingLevel(false); }} className="text-xs text-accent-gold font-bold hover:underline">Agregar</button>
          <button onClick={() => setAddingLevel(false)} className="text-xs text-wood-400">✕</button>
        </div>
      ) : (
        <button onClick={() => setAddingLevel(true)} className="flex items-center gap-1.5 text-xs font-bold text-accent-gold hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar nivel de complejidad</button>
      )}
      ...
      <div className="space-y-2">
        {config.engraveMaterials.map((m, i) => (
          <div key={i} className="flex items-center gap-3 px-4 py-2 bg-sand-50 dark:bg-wood-900/50 rounded-lg">
      ...
      {addingMat ? (
        <div className="flex items-center gap-2 px-4 py-2 bg-accent-gold/5 border border-accent-gold/20 rounded-lg">
          ...
          <button onClick={() => { if (!newMat.trim()) return; onChange({ engraveMaterials: [...config.engraveMaterials, { label: newMat, enabled: true }] }); setNewMat(''); setAddingMat(false); }} className="text-xs text-accent-gold font-bold hover:underline">Agregar</button>
          <button onClick={() => setAddingMat(false)} className="text-xs text-wood-400">✕</button>
        </div>
      ) : (
        <button onClick={() => setAddingMat(true)} className="flex items-center gap-1.5 text-xs font-bold text-accent-gold hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar material</button>
      )}
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      {addingShape ? (
        ...
        <button onClick={() => setAddingShape(true)} className="flex items-center gap-1.5 text-xs font-bold text-accent-gold hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar forma</button>
      )}
      ...
      {addingExtra ? (
        <div className="flex items-center gap-2 px-4 py-2 bg-accent-gold/5 border border-accent-gold/20 rounded-lg">
          ...
          <input value={newExtra.desc} onChange={e => setNewExtra(p => ({ ...p, desc: e.target.value }))} placeholder="Descripción" className="flex-1 text-xs bg-transparent outline-none text-wood-400" />
          <input type="number" value={newExtra.priceExtra} onChange={e => setNewExtra(p => ({ ...p, priceExtra: Number(e.target.value) }))} className="w-16 px-2 py-1 text-right text-sm border border-wood-200 rounded outline-none" placeholder="$" />
          <button onClick={() => { if (!newExtra.label.trim()) return; onChange({ boardExtras: [...config.boardExtras, { key: newExtra.label.toLowerCase().replace(/\s+/g, '_'), ...newExtra, enabled: true }] }); setNewExtra({ label: '', desc: '', priceExtra: 0 }); setAddingExtra(false); }} className="text-xs text-accent-gold font-bold hover:underline">Agregar</button>
          <button onClick={() => setAddingExtra(false)} className="text-xs text-wood-400">✕</button>
        </div>
      ) : (
        <button onClick={() => setAddingExtra(true)} className="flex items-center gap-1.5 text-xs font-bold text-accent-gold hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar extra</button>
      )}
      ...
      {addingFinish ? (
        <div className="flex items-center gap-2 px-4 py-2 bg-accent-gold/5 border border-accent-gold/20 rounded-lg">
          ...
          <button onClick={() => { if (!newFinish.trim()) return; onChange({ boardFinishes: [...config.boardFinishes, { label: newFinish, enabled: true }] }); setNewFinish(''); setAddingFinish(false); }} className="text-xs text-accent-gold font-bold hover:underline">Agregar</button>
          <button onClick={() => setAddingFinish(false)} className="text-xs text-wood-400">✕</button>
        </div>
      ) : (
        <button onClick={() => setAddingFinish(true)} className="flex items-center gap-1.5 text-xs font-bold text-accent-gold hover:underline"><Plus className="w-3.5 h-3.5" /> Agregar acabado</button>
      )}
    </div>
  );
}
