...
const ToggleSwitch: React.FC<{ enabled: boolean; onChange: (v: boolean) => void }> = ({ enabled, onChange }) => (
  <div
    ...
    className={`w-9 h-5 rounded-[var(--radius-badge)] p-0.5 transition-colors cursor-pointer ${enabled ? 'bg-green-500' : 'bg-wood-200'}`}
  >
    <div className={`w-4 h-4 rounded-[var(--radius-badge)] bg-[var(--surface)] shadow transition-transform ${enabled ? 'translate-x-4' : 'translate-x-0'}`} />
  </div>
);
...
const SaveButton: React.FC<{ onClick: () => void; saving: boolean; hasChanges: boolean }> = ({ onClick, saving, hasChanges }) => (
  <button
    ...
    className="flex items-center gap-1.5 px-4 py-2 text-xs bg-[var(--text)] text-[var(--bg)] rounded-[var(--radius-card)] hover:bg-[var(--text)] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
  >
...
const MetricCard: React.FC<{ label: string; value: string; sub?: string; highlight?: boolean }> = ({ label, value, sub, highlight }) => (
  <div className="bg-[var(--surface)] rounded-[var(--radius-card)] border border-[var(--border)] p-4">
    <p className={`text-lg ${highlight ? 'text-[var(--success)]' : 'text-[var(--text)]'}`}>{value}</p>
  ...
  return (
    <div className="flex gap-6 relative">
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className={`fixed top-4 right-4 z-50 flex items-center gap-2 px-4 py-3 rounded-[var(--radius-card)] shadow-lg text-xs font-medium ${
              toast.type === 'success'
                ? 'bg-[var(--success-subtle)] text-[var(--success)] border border-green-200'
                : 'bg-[var(--error-subtle)] text-[var(--error)] border border-red-200'
            }`}
          >
      ...
      <div className="w-48 flex-shrink-0 hidden lg:block">
        ...
        {hasChanges && (
          <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-[var(--radius-card)]">
            <p className="text-[10px] text-amber-700 font-medium">Cambios sin guardar</p>
            <div className="flex gap-2 mt-2">
              <button
                onClick={saveConfig}
                disabled={saving}
                className="flex-1 flex items-center justify-center gap-1 px-2 py-1.5 text-[10px] bg-[var(--text)] text-[var(--bg)] rounded-[var(--radius-button)] hover:bg-[var(--text)] disabled:opacity-50"
              >
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-1 px-2 py-1.5 text-[10px] text-[var(--text-secondary)] bg-[var(--surface)] border border-[var(--border)] rounded-[var(--radius-button)] hover:bg-[var(--surface2)]"
              >
                Deshacer
              </button>
            </div>
      ...
      <div className="flex-1 space-y-6 min-w-0">
        ...
        {/* ═══ GENERAL ═══════════════════════════════════ */}
        {section === 'general' && (
          <div className="bg-[var(--surface)] rounded-[var(--radius-card)] border border-[var(--border)] p-6 space-y-5">
              ...
              <div>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <input
                    ...
                    className="w-28 px-3 py-2 text-xs border border-[var(--border)] rounded-[var(--radius-card)] text-center outline-none focus:border-[var(--accent)]"
                  />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" value={config.points_per_mxn} onChange={e => updateField('points_per_mxn', Number(e.target.value))} min={1}
                    className="w-24 px-3 py-2 text-xs border border-[var(--border)] rounded-[var(--radius-card)] text-center outline-none focus:border-[var(--accent)]" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1">
                    <input type="number" value={config.min_redeem_points} onChange={e => updateField('min_redeem_points', Number(e.target.value))} min={1}
                      className="w-20 px-2 py-2 text-xs border border-[var(--border)] rounded-[var(--radius-card)] text-center outline-none focus:border-[var(--accent)]" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <select value={config.evaluation_period} onChange={e => updateField('evaluation_period', e.target.value)}
                    className="w-full px-3 py-2 text-xs border border-[var(--border)] rounded-[var(--radius-card)] bg-[var(--surface)] text-[var(--text)] outline-none focus:border-[var(--accent)]">
                ...
                {config.tiers.map((tier, index) => (
                  <div key={tier.id} className="px-5 py-4">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="w-8 h-8 rounded-[var(--radius-card)] flex-shrink-0"
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {config.tiers.length > 2 && (
                          <button onClick={() => removeTier(index)} className="text-[10px] text-[var(--error)] hover:text-[var(--error)]">
                            <Trash2 size={12} />
                          </button>
                        )}
                      </div>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                              <div>
                                ...
                                <input value={tier.id} onChange={e => updateTier(index, { id: e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, '') })}
                                  className="w-full px-2 py-1.5 text-xs border border-[var(--border)] rounded-[var(--radius-card)] outline-none focus:border-[var(--accent)]" />
                              </div>
                              <div>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                              ...
                              <div>
                                ...
                                <select value={tier.upgrade_gift || ''} onChange={e => updateTier(index, { upgrade_gift: e.target.value || null })}
                                  className="w-full px-2 py-1.5 text-xs border border-[var(--border)] rounded-[var(--radius-card)] bg-[var(--surface)] outline-none focus:border-[var(--accent)]">
                              ...
                              <div className="flex flex-wrap gap-2 mb-2">
                                {COLOR_PRESETS.map(preset => (
                                  <button key={preset.label}
                                    ...
                                    className="flex items-center gap-1.5 px-2 py-1 rounded-[var(--radius-button)] border border-[var(--border)] hover:border-[var(--accent)] transition-colors">
        ...
        {/* ═══ EMAILS ════════════════════════════════════ */}
        {section === 'emails' && (
          <div className="bg-[var(--surface)] rounded-[var(--radius-card)] border border-[var(--border)] p-6 space-y-1">
              ...
              <div key={i} className="flex items-center justify-between py-3 border-b border-[var(--border)] last:border-0">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {em.live
                      ? <span className="text-[8px] bg-[var(--success-subtle)] text-[var(--success)] px-1.5 py-0.5 rounded-[var(--radius-badge)] border border-green-200">Live</span>
                      : <span className="text-[8px] bg-[var(--surface2)] text-[var(--text-muted)] px-1.5 py-0.5 rounded-[var(--radius-badge)] border border-[var(--border)]">Pendiente</span>
                    }
                  </div>
            ...
            <div className="bg-[var(--surface)] rounded-[var(--radius-card)] border border-[var(--border)] p-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                {config.tiers.map(tier => (
                  <div key={tier.id} className="rounded-[var(--radius-card)] p-4 text-white relative overflow-hidden"
                    style={{ background: `linear-gradient(135deg, ${tier.colors.gradient_from}, ${tier.colors.gradient_via}, ${tier.colors.gradient_to})` }}>
      ...
      {hasChanges && (
        <div className="fixed bottom-0 left-0 right-0 lg:hidden bg-[var(--surface)] border-t border-[var(--border)] p-3 flex gap-2 z-40">
          <button onClick={saveConfig} disabled={saving}
            className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2.5 text-xs bg-[var(--text)] text-[var(--bg)] rounded-[var(--radius-card)] hover:bg-[var(--text)] disabled:opacity-50">
            {saving ? <Loader2 size={12} className="animate-spin" /> : <Save size={12} />}
            Guardar cambios
          </button>
          <button onClick={() => { setConfig(originalConfig); setEditingTierIndex(null); }}
            className="px-4 py-2.5 text-xs text-[var(--text-secondary)] bg-[var(--surface)] border border-[var(--border)] rounded-[var(--radius-card)] hover:bg-[var(--surface2)]">
            Deshacer
          </button>
        </div>
      )}
