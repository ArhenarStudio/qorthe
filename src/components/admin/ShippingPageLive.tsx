  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-serif text-3xl text-[var(--admin-text)]">Centro de Envíos</h1>
          <p className="text-[14px] text-[var(--admin-text-secondary)] mt-1">
        ...
        <button
          onClick={fetchOrders}
          disabled={loading}
          className="flex items-center gap-2 px-4 py-2 bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-lg text-[13px] text-[var(--admin-text-secondary)] hover:bg-[var(--admin-surface2)] transition-colors disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div className="bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-lg p-5">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-amber-50 flex items-center justify-center">
              <Clock size={18} className="text-amber-600" />
            </div>
            <div>
              <p className="text-[24px] font-bold text-[var(--admin-text)]">{pendingOrders.length}</p>
              <p className="text-[12px] text-[var(--admin-muted)]">Pendientes de envío</p>
            </div>
          </div>
        </div>
        <div className="bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-lg p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[24px] font-bold text-[var(--admin-text)]">{shippedOrders.length}</p>
              <p className="text-[12px] text-[var(--admin-muted)]">Enviados</p>
            </div>
          </div>
        </div>
        <div className="bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-lg p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[12px] text-[var(--admin-muted)]">Total órdenes</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Tabs */}
      <div className="flex gap-1 mb-6 bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-lg p-1 w-fit">
        <button
          onClick={() => setActiveTab("pending")}
          className={`px-4 py-2 rounded text-[13px] font-medium transition-colors ${
            activeTab === "pending"
              ? "bg-[var(--admin-text)] text-white"
              : "text-[var(--admin-text-secondary)] hover:bg-[var(--admin-surface2)]"
          }`}
        >
      ...
      {loading && (
        <div className="text-center py-16">
          <div className="animate-spin w-8 h-8 border-2 border-[var(--admin-text-secondary)] border-t-transparent rounded-full mx-auto mb-4" />
          <p className="text-[14px] text-[var(--admin-text-secondary)]">Cargando órdenes...</p>
        </div>
      )}

      {/* Empty state */}
      {!loading && orders.length === 0 && (
        <div className="bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-lg p-12 text-center">
          {activeTab === "pending" ? (
            <>
              <CheckCircle size={40} className="mx-auto mb-4 text-green-400" />
              <h2 className="font-serif text-xl text-[var(--admin-text)] mb-2">
                Todo al día
              </h2>
              <p className="text-[14px] text-[var(--admin-text-secondary)]">
                No hay pedidos pendientes de envío.
              </p>
            </>
          ) : (
            <>
              <Package size={40} className="mx-auto mb-4 text-[var(--admin-muted)]" />
              <h2 className="font-serif text-xl text-[var(--admin-text)] mb-2">
                Sin envíos aún
              </h2>
              <p className="text-[14px] text-[var(--admin-text-secondary)]">
        ...
        <div className="space-y-3">
          {orders.map((order) => {
            ...
            return (
              <div
                key={order.id}
                className="bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-lg overflow-hidden"
              >
                {/* Order row — clickable header */}
                <button
                  onClick={() => setExpandedOrder(isExpanded ? null : order.id)}
                  className="w-full flex items-center gap-4 p-4 hover:bg-[var(--admin-surface2)] transition-colors text-left"
                >
                  {/* Status dot */}
                  <div
                  ...
                  <div className="w-28 flex-shrink-0">
                    <p className="font-bold text-[14px] text-[var(--admin-text)]">
                      #{order.display_id}
                    </p>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] text-[var(--admin-text)] truncate">
                  ...
                  <div className="hidden md:block flex-1 min-w-0">
                    <p className="text-[13px] text-[var(--admin-text-secondary)] truncate">
                  ...
                  <div className="w-8 flex-shrink-0 text-center">
                    {order.fulfillment ? (
                      ...
                      <ChevronUp size={16} className="text-[var(--admin-muted)] mx-auto" />
                    ) : (
                      <ChevronDown size={16} className="text-[var(--admin-muted)] mx-auto" />
                    )}
                  </div>
                </button>
                ...
                {/* Expanded detail */}
                {isExpanded && (
                  <div className="border-t border-[var(--admin-border)] p-5 bg-[var(--admin-surface2)]">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      {/* Left: Items */}
                      <div>
                        <h3 className="text-[11px] uppercase tracking-wider text-[var(--admin-muted)] mb-3 font-semibold">
                          Artículos
                        </h3>
                        <div className="space-y-2">
                          {order.items.map((item) => (
                            <div
                              key={item.id}
                              className="flex items-center gap-3 bg-[var(--admin-surface)] rounded p-2 border border-[var(--admin-border)]"
                            >
                              {item.thumbnail ? (
                                <img
                                  src={item.thumbnail}
                                ...
                                <div className="w-10 h-10 rounded bg-[var(--admin-border)]" />
                              )}
                              <div className="flex-1 min-w-0">
                        ...
                        {/* Totals */}
                        <div className="mt-3 pt-3 border-t border-[var(--admin-border)] space-y-1">
                          <div className="flex justify-between text-[12px]">
                            <span className="text-[var(--admin-muted)]">Productos</span>
                            <span className="text-[var(--admin-text)]">{formatPrice(order.item_total)}</span>
                          </div>
                          <div className="flex justify-between text-[12px]">
                            <span className="text-[var(--admin-muted)]">Envío</span>
                            <span className="text-[var(--admin-text)]">{formatPrice(order.shipping_total)}</span>
                          </div>
                          <div className="flex justify-between text-[13px] font-bold pt-1 border-t border-[var(--admin-border)]">
                            <span className="text-[var(--admin-text)]">Total</span>
                            <span className="text-[var(--admin-text)]">{formatPrice(order.total)}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="bg-[var(--admin-surface)] rounded p-4 border border-[var(--admin-border)] space-y-1">
                          <p className="text-[14px] font-semibold text-[var(--admin-text)]">
                            {addr.first_name} {addr.last_name}
                          </p>
                          <p className="text-[13px] text-[var(--admin-text-secondary)]">{addr.address_1}</p>
                          {addr.address_2 && (
                            <p className="text-[13px] text-[var(--admin-text-secondary)]">{addr.address_2}</p>
                          )}
                          <p className="text-[13px] text-[var(--admin-text-secondary)]">
                            {addr.city}, {addr.province} CP {addr.postal_code}
                          </p>
                          {addr.phone && (
                            <p className="text-[13px] text-[var(--admin-text-secondary)]">Tel: {addr.phone}</p>
                          )}
                          <p className="text-[12px] text-[var(--admin-muted)] pt-1">{order.email}</p>
                        </div>

                        {order.shipping_method && (
                          <div className="mt-3 bg-[var(--admin-surface)] rounded p-3 border border-[var(--admin-border)]">
                            <p className="text-[11px] uppercase tracking-wider text-[var(--admin-muted)] mb-1">
                              Método seleccionado
                            </p>
                        ...
                        {!order.fulfillment && (
                          <div>
                                ...
                                <label
                                  ...
                                  className={`flex items-center gap-3 p-3 rounded border cursor-pointer transition-colors ${
                                    selectedCarrier === c.value
                                      ...
                                      : "bg-[var(--admin-surface)] border-[var(--admin-border)] text-[var(--admin-text)] hover:border-[var(--admin-text-secondary)]"
                                  }`}
                                >
                                  <input
                                    type="radio"
                            ...
                            <button
                              ...
                              className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-[var(--admin-text)] text-white text-[13px] font-semibold rounded hover:bg-[#3e3226] transition-colors disabled:opacity-50"
                            >
                              {isGenerating ? (
                                <>
